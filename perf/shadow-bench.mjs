...
const VARIANTS = arg('variants', 'formsOld,formsNew').split(',')

const TYPES = { '.html': 'text/html', '.js': 'text/javascript', '.mjs': 'text/javascript' }

const server = createServer((req, res) => {
  const path = (req.url ?? '/').split('?')[0]
  const file = path === '/pixi.mjs' ? PIXI : join(HERE, path === '/' ? 'shadow-bench.html' : path)
  readFile(file).then(
    (buf) => {
      res.writeHead(200, {
...
const url = `http://127.0.0.1:${String(PORT)}/shadow-bench.html?w=${String(W)}&h=${String(H)}&sprites=${String(SPRITES)}&forms=${String(Number(arg('forms', 200)))}`
...
console.log(
  `scene      ${String(info.drawables)} drawables (${String(info.sprites)} bodies, ${String(info.forms)} built forms)`,
)
console.log(`sampling   ${String(SAMPLES)} samples x ${String(FRAMES)} frames, round robin\n`)
console.log('variant      median      min      max   spread    vs old')
const base = median(res['formsOld'] ?? [])
for (const [name, arr] of Object.entries(res)) {
  const m = median(arr)
  const lo = Math.min(...arr)
  const hi = Math.max(...arr)
  const delta = res['formsOld'] === undefined || name === 'formsOld' ? '' : f(m - base)
