...
// The recognizer adds a few a day and the endpoint lists them oldest first: after dozens of
// days the page would open on the first week. Newest first, a dozen, and a count of the rest.
const CUSTOMS_SHOWN = 12
...
export function CustomsPage({ store }: Pick<PageProps, 'store'>) {
  ...
  const shown = rows.slice(-CUSTOMS_SHOWN).reverse()
  return (
    <>
      ...
      {rows.length > CUSTOMS_SHOWN && (
        <p className="feed-empty">And {rows.length - CUSTOMS_SHOWN} more from earlier days.</p>
      )}
    </>
  )
}
