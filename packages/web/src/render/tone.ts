...
export function toneReducer(
  prev: { graveUntil: number },
  evts: SimEvent[],
  tick: number,
): { graveUntil: number } {
  let graveUntil = prev.graveUntil
  for (const ev of evts) {
    if ((GRAVE_EVENTS as readonly string[]).includes(ev.type))
      graveUntil = Math.max(graveUntil, tick + GRAVE_HOLD_TICKS)
    else if ((GRAVE_AFTERMATH as readonly string[]).includes(ev.type))
      graveUntil = Math.max(graveUntil, tick + AFTERMATH_HOLD_TICKS)
  }
