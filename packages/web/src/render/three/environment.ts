...
export function createEnvironment(scene: Scene) {
  ...
  return {
    update(
      ...
      sun.intensity = daylight * (wet ? 0.24 : 3.1 + golden * 0.7) + moonlight * (wet ? 0.2 : 0.65)
      ...
      sky.intensity = 0.2 + daylight * (wet ? 0.35 : 1.2 - golden * 0.15)
      ...
      for (const slot of lights) {
        ...
        if (f) {
          ...
          slot.power = state.structures[f.id]?.kind === 'lamp_post' ? 17 : 24
        }
