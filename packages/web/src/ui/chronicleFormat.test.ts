...
describe('describeEvent', () => {
  ...
  it('renders speech, death, and completion with resolved names', () => {
    expect(
      describeEvent(
        ev('agent_spoke', { agentId: 'farmer', text: 'The wheat is in.', x: 1, y: 1 }),
        state,
      ),
    ).toBe('Wren: "The wheat is in."')
    expect(describeEvent(ev('agent_died', { agentId: 'farmer', cause: 'hunger' }), state)).toBe(
      'Wren starved.',
    )
    expect(describeEvent(ev('structure_completed', { id: 's1' }), state)).toBe(
      'The storehouse is finished.',
    )
  })
  ...
  it('covers the remaining viewer-worthy subset', () => {
    ...
    expect(describeEvent(ev('agent_tended', { agentId: 'farmer' }), state)).toBe(
      'Someone sat with Wren.',
    )
    expect(describeEvent(ev('action_completed', { agentId: 'farmer', verb: 'give' }), state)).toBe(
      'Wren gave something away.',
    )
  })
  ...
  it('hides plumbing and unknown future event types', () => {
    ...
    expect(
      describeEvent(ev('action_completed', { agentId: 'farmer', verb: 'fish' }), state),
    ).toBeNull()
