...
describe('describeEvent', () => {
  ...
  it('renders speech, death, and completion with resolved names', () => {
    ...
    expect(describeEvent(ev('agent_died', { agentId: 'farmer', cause: 'starvation' }), state))
      .toBe('Wren has died (starvation).')
  ...
  it('covers the remaining viewer-worthy subset', () => {
    ...
    expect(describeEvent(ev('agent_tended', { agentId: 'farmer' }), state)).toBe('Wren was tended.')
