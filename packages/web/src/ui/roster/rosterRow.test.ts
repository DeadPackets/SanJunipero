...
function world(seeds: AgentSeed[], structures: Structure[] = [], tick = 0): WorldState {
...
describe('the same person, five days apart, is not the same row', () => {
  const structures = [
    struct({ id: 'house', kind: 'house', x: 10, y: 12, w: 2, h: 2, owner: 'amara' }),
  ]
    ...
    [
      {
        ...
        x: 10,
        y: 12,
        skills: { farming: 9, carpentry: 4 },
        activity: { verb: 'build', ticksRemaining: 20 },
      },
  ...
  it('the RENDERED row differs — a row that reads the same on both is the defect', () => {
    const render = (r: RosterRow2): string =>
      renderToStaticMarkup(
        createElement(RosterRowView, { row: r, open: false, onToggle: () => {} }),
      )
...
describe('portrait — three honest fallbacks, never a broken image', () => {
  ...
  const atlas: AssetRecord = {
    ...
    seq: 2,
    class: 'rig-part',
    desc: 'character sheet v4: amara',
    kind: 'character:amara',
    footprint: { w: 1, h: 1 },
    widthPx: 2000,
    heightPx: 3400,
    status: 'ready',
  ...
  it('prefers the painted face, then the sprite bust, then the initial', () => {
    expect(rosterRows2(state, [portrait, atlas], null, 0)[0]!.portrait).toEqual({
      url: '/assets/p1.png',
    })
...
describe('mood — one table, not two', () => {
  ...
  it('an event the log recorded reaches the face', () => {
    const state = world([{ id: 'amara', name: 'Amara' }])
    const hit = [
      { seq: 1, tick: 0, type: 'agent_attacked', payload: { targetId: 'amara' } } as SimEvent,
    ]
...
describe('sortRoster — a preference, never a ranking', () => {
  const rows = rosterRows2(
    world(
      ...
      [struct({ id: 'well', kind: 'well', x: 1, y: 0 })],
    ),
    [],
    null,
    0,
...
describe('MOOD_GLYPH — a drawn face, never a character from the reader’s font', () => {
  it('is total over EXPRESSIONS and paints only MASTER_PALETTE members', () => {
    ...
    for (const fill of MOOD_GLYPH_PALETTE)
...
describe('the rendered row', () => {
  const state = world(
    ...
    [struct({ id: 'well', kind: 'well', x: 11, y: 11 })],
    DAY_TICKS,
  )
  ...
  it('★ says who is NEAR, never who someone is "with" — that word belongs to the bonds', () => {
    ...
    const alone = rosterRows2(
      world([{ id: 'amara', name: 'Amara', x: 10, y: 10 }], [], DAY_TICKS),
      [],
      null,
      DAY_TICKS,
    )[0]!
