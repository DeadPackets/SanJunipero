...
const agent = (over: Record<string, unknown>) => ({
  id: 'a', name: 'A', x: 0, y: 0, ageDays: 30 * 364, alive: true, asleep: false,
  collapsedSinceTick: null, activity: null, needs: { hunger: 80, energy: 80, warmth: 80, social: 80 },
  hp: 10, injuries: [], skills: {}, ...over,
})
...
describe('bustStyle', () => {
  const rec: AssetRecord = {
    id: 'asset_omar', seq: 1, class: 'rig-part', desc: 'character sheet v4: omar',
    kind: 'character:omar', footprint: { w: 1, h: 1 }, widthPx: 2000, heightPx: 3400,
  ...
  it('crops the idle cell around the head at the requested size', () => {
    ...
    expect(b.backgroundSize).toBe(`${Math.round(2000 * k * 100) / 100}px ${Math.round(3400 * k * 100) / 100}px`)
    // x centers the feet column, y starts at the figure top (feetY - figureH + cell y)
    const headX = (400 + 170) * k
    const topY = (850 + 5) * k
    expect(b.backgroundPosition).toBe(`${Math.round((24 - headX) * 100) / 100}px ${Math.round(-topY * 100) / 100}px`)
  })
