...
import {
  AT_RADIUS_TILES, TERRAIN_WORDS, placeOf, structureWords,
} from './place.js'

const TILE: Record<string, TileId> = {
  grass: 0, dirt: 1, water: 2, forest: 3, rock: 4, sand: 5, farmland: 6, road: 7,
}
...
function world(over: {
  ...
  for (const a of over.agents ?? []) {
    agents[a.id!] = {
      id: a.id, name: a.name ?? a.id, x: a.x ?? 0, y: a.y ?? 0, alive: true, asleep: false,
  ...
  return {
    tick: 0, terrain: over.terrain ?? flat(), weather: { kind: 'sunny', temperatureC: 12 },
    agents: agents as WorldState['agents'], structures, items: {}, crops: {},
...
describe('placeOf — the ground under them', () => {
  it('water-edge earth is the river bank', () => {
    ...
    t[20]![10] = TILE.water!
    t[20]![11] = TILE.dirt!
    expect(placeOf(world({ terrain: t, agents: [{ id: 'a', x: 11, y: 20 }] }), 'a'))
      .toEqual({ words: 'on the river bank', kind: 'on' })
  })

  it('dirt away from any water is just ground, not a bank', () => {
    const t = flat()
    t[20]![11] = TILE.dirt!
    expect(placeOf(world({ terrain: t, agents: [{ id: 'a', x: 11, y: 20 }] }), 'a').kind).toBe('out')
  })

  it('names the forest and the fields', () => {
    for (const [tile, words] of [[TILE.forest!, 'in the forest'], [TILE.farmland!, 'in the fields']] as const) {
      const t = flat()
      t[20]![11] = tile
      expect(placeOf(world({ terrain: t, agents: [{ id: 'a', x: 11, y: 20 }] }), 'a'))
        .toEqual({ words, kind: 'on' })
    }
  })

  it('open grass with nothing near is out past the edge of town', () => {
    expect(placeOf(world({ agents: [{ id: 'a', x: 5, y: 5 }] }), 'a'))
      .toEqual({ words: 'out past the edge of town', kind: 'out' })
  })
...
describe('structureWords', () => {
  ...
  it('never leaks a slug — a fire pit is two words and a standing stone is two more', () => {
    expect(structureWords(state, struct({ id: 'f', kind: 'fire_pit', x: 0, y: 0 })))
      .toBe('the fire pit')
    expect(structureWords(state, struct({ id: 's', kind: 'standing_stone', x: 0, y: 0 })))
      .toBe('the standing stone')
    expect(structureWords(state, struct({ id: 'x', kind: 'some_new_kind', x: 0, y: 0 })))
      .toBe('the some new kind')
  })
})
...
describe('no output is ever machine vocabulary', () => {
  ...
  const structures = makeCityTemplate(ANCHOR).structures.map((s, i) => struct({
    id: `structure_${s.kind}_${i}`, kind: s.kind,
    x: ANCHOR.x + s.dx, y: ANCHOR.y + s.dy, w: s.w, h: s.h,
    ...(s.owner === null ? {} : { owner: s.owner }),
  }))
  const names = Object.fromEntries(FOUNDER_IDS.map((id) => [id, id[0]!.toUpperCase() + id.slice(1)]))

  it('over 200 positions across the eleven-building town, says only words', () => {
    let sawAt = 0, sawOut = 0
    for (let i = 0; i < 200; i++) {
      const x = i % 40, y = 5 + ((i * 7) % 34)
      const t = flat()
