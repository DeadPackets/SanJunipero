...
for (const [i, c] of seCands.entries()) {
  ...
  console.log(
    `se/idle candidate ${i}: score=${v.score} d(sw/idle)=${dist.toFixed(3)} $${c.costUsd.toFixed(3)} — ${v.notes}`,
  )
  ...
  else if (seWinner.score < 7) {
    if (v.score > seWinner.score) seWinner = cand
...
for (const [i, c] of wbCands.entries()) {
  ...
  if (!wbWinner || v.score > wbWinner.score)
    wbWinner = { raw: c.png, cell: await postProcessCell(c.png), score: v.score }
}
...
console.log(
  `se/walk-b winner: score=${wbWinner!.score} d(se/idle)=${dIdle.toFixed(3)} d(se/walk-a)=${dWalkA.toFixed(3)}` +
    (dIdle > STRAIGHT_DUPE && dWalkA > STRAIGHT_DUPE
...
function matrix(mirror: boolean): string {
  ...
  const lines = labels.map((la) =>
    [
      la.padEnd(12),
      ...labels.map((lb) =>
        cellDistance(cells.get(la)!, mirror ? mirrorX(cells.get(lb)!) : cells.get(lb)!)
          .toFixed(3)
          .padStart(11),
      ),
    ].join(' '),
