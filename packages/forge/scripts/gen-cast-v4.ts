...
import {
  ...
  sliceStrip, estimatePitch, opaqueBbox, anchorToCanvas, assembleGrid, downscaleMajority,
  cellDistance, pairwiseMedian,
  type GateFailure,
} from '../src/sheet.js'
import {
...
async function candidate(dir: string, key: string, prompt: string, refs: Buffer[], size: string, reserve: number): Promise<Buffer> {
...
function conceptPrompt(m: CastMember): string {
  return 'High-detail character concept art in a warm storybook box-art style, full body, ' +
...
async function runCharacter(m: CastMember): Promise<void> {
  ...
  currentAsset = m.id
  const report: string[] = []
  const push = (line: string) => { report.push(line); console.log(`  ${line}`) }
  console.log(`\n== ${m.id} ==`)
  ...
  for (let i = 0; i < (masterKeys ? masterKeys.length : 3); i++) {
    ...
    try {
      ...
      const seH = (() => { const b = opaqueBbox(seHi)!; return b.y1 - b.y0 + 1 })()
      ...
      push(`${key}: sliced OK, pitch=${pitches.map(p => p.toFixed(2)).join('/')}, front-back distance=${frontBack.toFixed(3)}`)
      masters.push({ key, raw, se: seHi, ne: neHi, pitch, frontBack })
    } catch (e) {
      push(`${key}: process FAILED — ${String(e).slice(0, 200)}`)
    }
  }
  if (masters.length === 0) throw new Error(`${m.id}: both master candidates failed to slice into two figures`)
  ...
  async function genFrame(f: AuthoredFacing, p: WalkPose, i: number): Promise<FrameCand | null> {
    ...
    for (;;) {
      ...
      try {
        ...
        push(`${key}: gates=${cand.failures.length === 0 ? 'PASS' : cand.failures.map(x => `${x.gate}(${x.value.toFixed(3)})`).join(',')}`)
        return cand
      } catch (e) {
        push(`${key}: process FAILED — ${String(e).slice(0, 200)}`)
        return null
    ...
    if (stride.length > 0) {
      ...
      if (frameCands[f][pose].length < 2) {
        ...
        if (retry) {
          ...
          const altStride = strideGateV4(f, {
            'idle': masterGate[f],
  ...
  for (let i = 0; i < 2; i++) {
    ...
    try {
      ...
      if (bw !== TARGET_H) {
        const k = TARGET_H / bw
        hi = downscaleNearest(hi,
          Math.max(1, Math.round(hi.width * k)), Math.max(1, Math.round(hi.height * k)))
      }
      const failures = sleepCoherenceGateV4(masterGate.se, gateView(hi))
      push(`${key}: gates=${failures.length === 0 ? 'PASS' : failures.map(x => `${x.gate}(${x.value.toFixed(3)})`).join(',')}`)
  ...
  writeFileSync(`${DIR}/contact-sheet.png`, await encodePng(assembleGrid(contactRows, PREVIEW_CELL, PREVIEW_CELL)))
