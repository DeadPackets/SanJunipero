...
for (const d of [`${DURABLE}/raws`, `${DURABLE}/cells`, `${DURABLE}/master`, `${DURABLE}/gifs`])
...
reportLines.push(
  `master ${MASTER_KEY}: idle crops se=${idleSe.width}x${idleSe.height} ne=${idleNe.width}x${idleNe.height}, figureH=${TARGET_H} (NE height-normalized to SE)`,
)
...
async function genFrame(f: AuthoredFacing, p: WalkPose, i: number): Promise<FrameCand | null> {
  ...
  for (;;) {
    ...
    try {
      ...
      reportLines.push(`${key}: generation FAILED — ${String(e).slice(0, 200)}`)
    ...
    try {
      ...
      reportLines.push(`${key}: process FAILED — ${String(e).slice(0, 200)}`)
      console.log(`  ${key}: process FAILED — ${String(e).slice(0, 200)}`)
      return null
    }
  }
}
...
let sleepHi = processHiResCell(keyBg(await decodePng(readFileSync(sleepRawPath))))
{
  ...
  if (bw !== TARGET_H) {
    const k = TARGET_H / bw
    sleepHi = downscaleNearest(
      sleepHi,
      Math.max(1, Math.round(sleepHi.width * k)),
      Math.max(1, Math.round(sleepHi.height * k)),
    )
  }
}
