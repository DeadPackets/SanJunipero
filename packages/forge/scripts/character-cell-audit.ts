...
import {
  CRITERIA, HARD_FAIL_CRITERIA, criterionOf, type Criterion, type VisionVerdict,
...
type Unit = { id: string; founder: string; cell: string; path: string; commission: string; expectedFacing: string }
...
function keylessCheck(img: RawImage): Keyless {
  let offPalette = 0, semiAlpha = 0, opaque = 0
  for (let i = 0; i < img.data.length; i += 4) {
    const a = img.data[i + 3]!
    if (a === 0) continue
    opaque++
    if (a < 255) semiAlpha++
    if (!PALETTE_KEYS.has((img.data[i]! << 16) | (img.data[i + 1]! << 8) | img.data[i + 2]!)) offPalette++
  }
  return { offPalette, semiAlpha, opaque }
}

const ONLY_F = (process.env.FOUNDERS ?? FOUNDERS.map(f => f.id).join(',')).split(',').map(s => s.trim())
const ONLY_C = (process.env.CELLS ?? CELL_NAMES_V4.join(',')).split(',').map(s => s.trim())

function enumerate(): { units: Unit[]; missing: string[] } {
  const units: Unit[] = [], missing: string[] = []
...
console.log(`character-cell-audit ${DRY ? 'DRY PLAN' : REPORT ? 'REPORT (offline)' : 'LIVE'} — ${units.length} cells, `
  + `${units.length} calls, est $${(units.length * EST_COST_PER_VISION_CALL).toFixed(4)}`)
if (missing.length) { console.error(`missing cells: ${missing.join(', ')}`); process.exit(1) }
if (DRY) {
...
const verdictPath = (u: Unit): string => join(OUT, 'cell-verdicts', `${u.id.replace(/:/g, '_')}.json`)
...
if (REPORT) {
  for (const u of units) {
    if (!existsSync(verdictPath(u))) { console.error(`no verdict on disk for ${u.id}`); process.exit(1) }
    results.push({
      unit: u, keyless: kl(u), costUsd: 0,
  ...
  for (const u of units) {
    ...
    const { verdict, costUsd } = await judge({
      assetId: u.id, klass: 'character', sprite, commission: u.commission,
      expectedFacing: u.expectedFacing, attempt: 1,
    })
...
const holds = (v: VisionVerdict, c: Criterion): boolean => {
  ...
  return x !== undefined && (x.evidence.startsWith('not applicable') || (x.pass && x.score >= config.visionQa.minScore))
}
const verdicts = results.map(r => r.verdict)
const perCriterion = CRITERIA.map(c => {
  const vs = verdicts.filter(v => asked(v, c) && !criterionOf(v, c)!.evidence.startsWith('not applicable'))
...
const bad = (u: Unit, v: VisionVerdict): Criterion[] => EYE_ONLY.filter(c => {
  if (c === 'facing' && u.cell.startsWith('sleep-')) return false
  const x = criterionOf(v, c)
  return x !== undefined && !x.evidence.startsWith('not applicable')
...
const borderline = results.filter(r => !defects.includes(r)
  && CRITERIA.some(c => asked(r.verdict, c) && !holds(r.verdict, c)))
const offPaletteCells = results.filter(r => keylessBad(r.keyless))
const judgedPaletteBad = results.filter(r => (criterionOf(r.verdict, 'palette')?.score ?? 10) <= DEFECT_AT)
const judgedTranspBad = results.filter(r => (criterionOf(r.verdict, 'transparency')?.score ?? 10) <= DEFECT_AT)
...
const md = [
  ...
  '|---|---|---|---|---|',
  (() => { const r = passRates(db, 'character-cell'); return `| character-cell | ${r.firstPass.toFixed(3)} | ${r.withinRetries.toFixed(3)} | ${r.blocked.toFixed(3)} | ${r.n} |` })(),
  '',
  '## Per-founder',
  '',
  ...
  ...FOUNDERS.filter(f => ONLY_F.includes(f.id)).map(f => {
    ...
    return `| ${f.id} | ${rs.length} | ${rs.filter(r => r.verdict.overall === 'pass').length} | `
  ...
  '',
  defects.length === 0 ? '_empty_' : [
    `| cell | ${CRITERIA.join('/')} | off-palette / semi-alpha | failing | evidence |`,
    '|---|---|---|---|---|',
    ...defects.map(r => {
      const bs = bad(r.unit, r.verdict)
      return `| \`${r.unit.founder}/${r.unit.cell}\` | ${scores(r.verdict)} | `
        + `${r.keyless.offPalette} / ${r.keyless.semiAlpha} | ${bs.join(', ') || '—'} | `
        + `${bs.map(c => criterionOf(r.verdict, c)!.evidence).join(' ').replace(/\|/g, '/').slice(0, 220) || 'keyless count only'} |`
    }),
  ].join('\n'),
  '',
  '## Every cell',
  ...
  '|---|---|---|',
  ...results.map(r => `| \`${r.unit.founder}/${r.unit.cell}\` | **${r.verdict.overall}** | ${scores(r.verdict)} |`),
  '',
].join('\n')
