...
const grid: Cell[] = [...cells]
  ...
  .map(([x, y]) => ({
    x, y, key: roadAutotile({ n: on(x, y - 1), e: on(x + 1, y), s: on(x, y + 1), w: on(x - 1, y) }),
  }))
...
const minX = Math.min(...grid.map(sx)), maxX = Math.max(...grid.map(sx))
const minY = Math.min(...grid.map(sy)), maxY = Math.max(...grid.map(sy))
...
writeFileSync(json, JSON.stringify({
  version: 'v1-autotile-testmap', mapW: MAP_W, mapH: MAP_H, tileW: TILE_W, tileH: TILE_H,
  keysExercised: [...used].sort(), cells: grid,
}, null, 2))
console.log(`wrote ${png} (${map.width * SCALE}x${map.height * SCALE}) and ${json} — ` +
