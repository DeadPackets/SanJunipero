...
type Building = { id: string; kind: string; desc: string; fp: { w: number; h: number }; door: boolean }
...
const BUILDINGS: readonly Building[] = [
  { id: 'storehouse', kind: 'storehouse', fp: { w: 2, h: 2 }, door: true, desc: 'a sturdy communal storehouse of honey-wood planks on a cream-stone base, wide double doors, a small hay loft window under the roof peak' },
  { id: 'wagon', kind: 'wagon', fp: { w: 1, h: 2 }, door: false, desc: 'a settler travel wagon with a rounded cream canvas cover, honey-wood body and two big spoked wheels, standing still with no animals' },
  { id: 'shed', kind: 'shed', fp: { w: 1, h: 1 }, door: true, desc: 'a small honey-wood tool shed with a single plank door and a slanted roof' },
  { id: 'scaffolding', kind: 'scaffolding', fp: { w: 1, h: 1 }, door: false, desc: 'a construction scaffolding of honey-wood poles and cream canvas wraps, building-under-construction sprite' },
  { id: 'standing-stone', kind: 'standing_stone', fp: { w: 1, h: 1 }, door: false, desc: 'an ancient weathered warm-grey standing stone, taller than a person, softly rounded edges, faint moss at its base' },
]
const FILTER = (process.env.BUILDINGS ?? BUILDINGS.map(b => b.id).join(',')).split(',').map(s => s.trim())
...
async function generate(prompt: string, size: string, reserve: number): Promise<{ raw: Buffer; cost: number }> {
...
async function candidate(dir: string, asset: string, key: string, prompt: string): Promise<Buffer> {
  ...
  console.log(`  ${key}: generated ($${r.cost.toFixed(3)}), total spend $${budget.total.toFixed(3)}`)
  return r.raw
}
...
for (const b of RUN) {
  ...
  writeFileSync(`${DIR}/manifest.json`, JSON.stringify({
    version: 'v4-hires-building', kind: b.kind, footprint: b.fp,
  ...
  const pw = Math.max(1, Math.round(win.hi.width * k)), ph = Math.max(1, Math.round(win.hi.height * k))
  const previewBuf = await sharp(Buffer.from(win.hi.data.buffer, win.hi.data.byteOffset, win.hi.data.byteLength),
    { raw: { width: win.hi.width, height: win.hi.height, channels: 4 } })
    .resize(pw, ph, { kernel: 'lanczos3', fit: 'fill' }).png().toBuffer()
  writeFileSync(`${DIR}/preview-4x.png`, previewBuf)
  lines.push(`chosen: ${win.key} (picked by pixel pitch); anchor={w:${anchor.w},h:${anchor.h},x:${anchor.feetX},y:${anchor.feetY}}`)
  lines.push(`asset spend: $${(assetSpend[b.id] ?? 0).toFixed(3)}`)
  writeFileSync(`${DIR}/report.txt`, lines.join('\n'))
  writeFileSync(`${DIR}/spend.json`, JSON.stringify({ asset: `building-${b.id}`, spendUsd: assetSpend[b.id] ?? 0 }, null, 2))
