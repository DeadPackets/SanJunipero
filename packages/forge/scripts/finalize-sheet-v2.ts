...
for (let i = 0; i < 2; i++) {
  ...
  console.log(
    `sw/walk-b regen candidate ${i}: score=${v.score} $${costUsd.toFixed(4)} — ${v.notes}`,
  )
...
for (const p of POSES)
  for (const f of FACINGS) {
    const cell = await postProcessCell(
      readFileSync(`${DURABLE}/raws/${file(f, p)}`),
      CANVAS,
      FEET_Y,
    )
...
console.log(
  `sw/walk-b verify: d(sw/idle)=${dIdle.toFixed(3)} d(sw/walk-a)=${dWalkA.toFixed(3)}` +
    (dIdle > STRAIGHT_THR && dWalkA > STRAIGHT_THR
