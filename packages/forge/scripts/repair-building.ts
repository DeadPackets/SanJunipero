...
type Repair = {
  id: string; kind: string; desc: string; fp: { w: number; h: number }; door: boolean
  defect: string; note: string
}
...
const REPAIRS: readonly Repair[] = [
  {
    id: 'standing-stone', kind: 'standing_stone', fp: { w: 1, h: 1 }, door: false,
    desc: 'an ancient weathered warm-grey standing stone, taller than a person, softly rounded edges, faint moss at its base',
    defect: 'transparency 3, proportion 3, density 4, alignment 4 — a washed-out pale patch across the upper right, dithered noise, transparent holes inside the stone (long-standing user-flagged defect)',
  ...
  {
    id: 'shed', kind: 'shed', fp: { w: 1, h: 1 }, door: true,
    desc: 'a small honey-wood tool shed with a single plank door and a slanted roof',
    defect: 'alignment 3, seat 6 — the stacked log pile on the lower right overhangs the 1x1 footprint diamond',
  ...
  {
    id: 'storehouse', kind: 'storehouse', fp: { w: 2, h: 2 }, door: true,
    ...
    defect: 'palette 3, proportion 3, density 4, seat 2 — micro-dithered noise across the roof and walls at a finer scale than the project grid',
  ...
  {
    id: 'wagon', kind: 'wagon', fp: { w: 1, h: 2 }, door: false,
    ...
    defect: 'facing 4, density 6 — a rear door panel that contradicts the open canvas, and wheel spokes drawn finer than the project grid',
  ...
  {
    id: 'scaffolding', kind: 'scaffolding', fp: { w: 1, h: 1 }, door: false,
    desc: 'a construction scaffolding of honey-wood poles and cream canvas wraps, building-under-construction sprite',
    defect: 'alignment 6 — the lowest front vertical pole stops short of the ground diamond bottom vertex',
    note: 'extend the lowest front vertical pole downward so its foot ends level with the feet of the other corner posts.',
  },
]

const FILTER = (process.env.BUILDINGS ?? REPAIRS.map(r => r.id).join(',')).split(',').map(s => s.trim())
...
type Manifest = { version: string; kind: string; footprint: { w: number; h: number }; cell: { w: number; h: number; feetX: number; feetY: number } }

function editPrompt(r: Repair): string {
  return `${STYLE_PROMPT} Reproduce the LAST reference image EXACTLY — the same single building, ` +
...
async function preview(cell: RawImage, fp: { w: number; h: number }): Promise<Buffer> {
  ...
  return sharp(Buffer.from(cell.data.buffer, cell.data.byteOffset, cell.data.byteLength),
    ...
    .resize(Math.max(1, Math.round(cell.width * k)), Math.max(1, Math.round(cell.height * k)),
      { kernel: 'lanczos3', fit: 'fill' }).png().toBuffer()
}

type Row = {
  id: string; raw: string; rubric: string; scores: number[]; pixel: boolean
  failures: string[]; seat: number; seatEvidence: string; deployed: boolean; costUsd: number
}
...
const PROMOTE = (process.env.PROMOTE ?? '').split(',').map(s => s.trim()).filter(Boolean)

async function deploy(r: Repair, cellPng: Buffer, cell: RawImage, man: Manifest, rawKey: string, score: number, attempts: number, costUsd: number): Promise<void> {
  ...
  for (const root of [C5, ARCHIVE]) {
    ...
    if (!existsSync(d)) { console.log(`  (no copy at ${d})`); continue }
  ...
  const rec = codex.register({
    class: 'building', kind: r.kind, desc: r.desc, meta: JSON.stringify(man),
    footprint: r.fp, png: cellPng, widthPx: cell.width, heightPx: cell.height,
    status: 'ready', attempts, costUsd, score: Math.min(10, Math.max(1, score)),
  })
  writeFileSync(join(C13, 'buildings', r.id, 'deployed.json'),
    JSON.stringify({ id: r.id, raw: rawKey, assetId: rec.id, score, costUsd, at: rec.createdAt }, null, 2))
}
...
if (PROMOTE.length) {
  for (const r of REPAIRS.filter(x => PROMOTE.includes(x.id))) {
    ...
    const v = JSON.parse(readFileSync(join(out, 'verdicts.json'), 'utf8')) as
      { raw: string; rubric: VisionVerdict; align: { ok: boolean } }
    ...
    const man: Manifest = { version: 'v4-hires-building', kind: r.kind, footprint: r.fp, cell: cellAnchor(cell) }
    ...
    console.log(`  promoted ${r.id} from ${v.raw} (rubric ${v.rubric.overall}, mean ${score.toFixed(1)})`)
  }
...
for (const r of (PROMOTE.length || process.env.REPORT === '1' ? [] : RUN)) {
  ...
  const man: Manifest = { version: 'v4-hires-building', kind: r.kind, footprint: r.fp, cell: anchor }

  const { verdict, costUsd } = await judge({
    assetId, klass: 'building', sprite: cell, commission: r.desc, footprint: r.fp, attempt: 1,
  ...
  const seat = await judge({
    assetId: `seat:${r.id}`, klass: 'building', sprite: grid, attempt: 1,
    commission: `${r.kind} on its footprint diamond. ${SEAT_CRITERION_PROMPT}`,
    footprint: r.fp, expectedFacing: 'door to the south-west',
  })
  ...
  writeFileSync(join(C13, 'buildings', r.id, 'verdicts.json'), JSON.stringify(
    { id: r.id, raw: outKey, rubric: verdict, seat: seat.verdict, align }, null, 2))

  let deployed = false
  if (ok && DEPLOY) {
    const score = CRITERIA.reduce((s, c) => s + verdict.criteria[c].score, 0) / CRITERIA.length
    await deploy(r, cellPng, cell, man, outKey, score, Math.min(3, n), ledger.totalFor(assetId) - spentBefore)
    deployed = true
  }

  rows.push({
    id: r.id, raw: outKey, rubric: verdict.overall, pixel: align.ok, failures: align.failures,
    scores: CRITERIA.map(c => verdict.criteria[c].score),
    seat: seat.verdict.criteria.alignment.score, seatEvidence: seat.verdict.criteria.alignment.evidence,
    deployed, costUsd: ledger.totalFor(assetId) - spentBefore,
  })
  console.log(`  ${verdict.overall.padEnd(7)} pixel ${align.ok ? 'PASS' : 'FAIL'} seat ${seat.verdict.criteria.alignment.score} ` +
...
for (const r of REPAIRS) {
  ...
  const v = JSON.parse(readFileSync(join(out, 'verdicts.json'), 'utf8')) as
    { raw: string; rubric: VisionVerdict; seat: VisionVerdict; align: { ok: boolean; failures: string[] } }
  rows.push({
    id: r.id, raw: v.raw, rubric: v.rubric.overall, pixel: v.align.ok, failures: v.align.failures,
    scores: CRITERIA.map(c => v.rubric.criteria[c].score),
    seat: v.seat.criteria.alignment.score, seatEvidence: v.seat.criteria.alignment.evidence,
    deployed: existsSync(join(out, 'deployed.json')), costUsd: ledger.totalFor(`building:${r.id}`),
  })
}
rows.sort((a, b) => REPAIRS.findIndex(r => r.id === a.id) - REPAIRS.findIndex(r => r.id === b.id))
...
const md = [
  ...
  '|---|---|---|---|---|---|---|---|',
  ...rows.map(r => `| ${r.id} | \`${r.raw}\` | **${r.rubric}** | ${r.scores.join(' ')} | ` +
    `${r.pixel ? 'PASS' : '**FAIL**'} | ${r.seat} | ${r.deployed ? 'yes' : 'no'} | ${r.costUsd.toFixed(4)} |`),
  '',
  '## Defects repaired',
  '',
  ...REPAIRS.filter(r => rows.some(x => x.id === r.id)).map(r => `- \`${r.id}\` — ${r.defect}\n  - edit: ${r.note}`),
  '',
  '## Seat evidence',
  '',
  ...rows.map(r => `- \`${r.id}\` (${r.seat}): ${r.seatEvidence}`),
  '',
  ...rows.filter(r => r.failures.length).map(r => `- \`${r.id}\` pixel failures: ${r.failures.join('; ')}`),
  '',
].join('\n')
mkdirSync(join(C13, 'buildings'), { recursive: true })
