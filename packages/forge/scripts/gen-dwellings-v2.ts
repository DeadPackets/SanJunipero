...
const FACINGS_ARG = (process.env.DWELL_FACINGS ?? STRUCTURE_FACINGS.join(','))
  .split(',').map((s) => s.trim()).filter(Boolean) as StructureFacing[]
...
const FACING_CLAUSE: Record<StructureFacing, string> = {
  ...
  se: 'ORIENTATION: the building is turned NINETY DEGREES from the usual view. The FRONT of the '
...
type Subject = { id: string; kind: string; fp: { w: number; h: number }; ref: RefMode; desc: string }

const SUBJECTS: readonly Subject[] = [
  {
    id: 'house', kind: 'house', fp: { w: 2, h: 2 }, ref: 'swatch',
    desc:
  ...
  {
    id: 'cottage', kind: 'cottage', fp: { w: 3, h: 2 }, ref: 'swatch',
    desc:
  ...
  {
    id: 'cabin', kind: 'cabin', fp: { w: 2, h: 2 }, ref: 'swatch',
    desc:
  ...
  },
  {
    ...
    id: 'farmhouse', kind: 'farmhouse', fp: { w: 4, h: 2 }, ref: 'swatch',
    desc:
  ...
  {
    id: 'storehouse', kind: 'storehouse', fp: { w: 2, h: 2 }, ref: 'swatch',
    desc:
  ...
  for (const facing of FACINGS_ARG) {
    ...
    for (let i = 0; i < MAX_ATTEMPTS; i++) {
      ...
      if (existsSync(path)) { buf = readFileSync(path); console.log(`  ${key}: cached`) } else {
        ...
        const r = await generate(prompt(s, facing, refMode), refMode === 'anchor' ? STYLE_ANCHOR : swatch, label)
    ...
    rows.push(`| ${label} | ${s.fp.w}x${s.fp.h} | ${cellPx} | ${refMode} | ${GEN_PX}/${win.plan.factor} `
...
const md = ['# round 4 — four dwellings and a storehouse, in two facings', '',
