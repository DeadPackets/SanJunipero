...
for (const d of [GIFS_DIR, CROPS_DIR, REFS_DIR, 'packages/forge/out/refs-v2', `${OUT}/cells`])
...
function runV7(keyed: RawImage, pitch: number, name: string): Processed {
  ...
  if (c.despeckleRemoved > 0)
    console.log(
      `  ${name}: despeckle removed ${c.despeckleRemoved} px${c.despeckleRemoved > 2 ? ' ** FLAG >2 **' : ''}`,
    )
  return c
}
...
function place(img: RawImage, tx: number, ty: number): RawImage {
  ...
  if (dropped > 0)
    throw new Error(
      `place: ${dropped} opaque pixels dropped outside the ${CANVAS}x${CANVAS} canvas`,
    )
...
console.log(
  'per-cell metrics (cluster ambiguous% / dupRows / reconErr / sweep / snaps / exclDelta):',
)
...
for (const [name, path] of refInputs) {
  ...
  const m = sheetMetrics([c])
  metricLines.push(
    `${name.padEnd(12)} ${`${c.out.width}x${c.out.height}`.padEnd(8)} ${m.ambiguousPct.toFixed(1).padStart(5)}%  ${m.dupRowCount}  ${m.reconErr.toFixed(4)}  sweep=${c.sweepHits}  snaps=${c.outlineSnaps}o+${c.fillSnaps}f  excl=${c.exclDelta >= 0 ? '+' : ''}${c.exclDelta.toFixed(5)}`,
  )
  console.log(
    `${name}: pitch ${pitch.toFixed(2)}, natural ${c.out.width}x${c.out.height}, ambiguous ${m.ambiguousPct.toFixed(1)}%, sweep=${c.sweepHits}  snaps=${c.outlineSnaps}o+${c.fillSnaps}f  excl=${c.exclDelta >= 0 ? '+' : ''}${c.exclDelta.toFixed(5)}`,
  )
...
const sheet = assembleGrid(
  POSES.map((p) => FACINGS.map((f) => cells.get(label(f, p))!)),
  CANVAS,
  CANVAS,
)
...
for (const [f, p] of [
  ...
  writeFileSync(
    `${CROPS_DIR}/${file(f, p).replace('.png', '-4x.png')}`,
    await encodePng(upscaleNearest(cell, 4)),
  )
}
for (const f of FACINGS) {
  const frames = (['idle', 'walk-a', 'idle', 'walk-b'] as const).map((p) =>
    upscaleNearest(cells.get(label(f, p))!, 4),
  )
  ...
  const gif = await sharp(Buffer.from(stacked.buffer), {
    ...
    .gif({ delay: frames.map(() => 180), loop: 0 })
    .toBuffer()
  const meta = await sharp(gif, { animated: true }).metadata()
  if (meta.pages !== frames.length)
    throw new Error(`walk-${f}.gif: expected ${frames.length} pages, got ${meta.pages}`)
...
const STRAIGHT_THR = 0.36 * median,
  MIRROR_THR = 0.21 * median
const matrix = (mirror: boolean) =>
  distanceMatrix(
    labels.map((l) => ({ label: l, img: cells.get(l)! })),
    mirror,
  )
const rows = POSES.flatMap((p) =>
  duplicateReport(
    FACINGS.map((f) => ({ label: label(f, p), img: cells.get(label(f, p))! })),
    STRAIGHT_THR,
    MIRROR_THR,
  ),
)
const cols = FACINGS.flatMap((f) =>
  duplicateReport(
    POSES.map((p) => ({ label: label(f, p), img: cells.get(label(f, p))! })),
    STRAIGHT_THR,
    MIRROR_THR,
  ),
)
const report = [
  ...
  '',
  '== dupe findings ==',
  ...(rows.length + cols.length === 0
    ? ['none']
    : [
        ...rows.map(
          (fd) => `row  ${fd.a} ~ ${fd.b} d=${fd.distance.toFixed(3)} mirrored=${fd.mirrored}`,
        ),
        ...cols.map(
          (fd) => `col  ${fd.a} ~ ${fd.b} d=${fd.distance.toFixed(3)} mirrored=${fd.mirrored}`,
        ),
      ]),
].join('\n')
