...
const raws = readdirSync(join(dir, 'candidates'))
  .filter((f) => f.endsWith('-raw.png'))
  .sort()
const tiles: Buffer[] = []
for (const f of raws) {
  ...
  console.log(
    ...
    c.islands,
    'fill',
    c.opaqueFrac.toFixed(2),
    'rank',
    candidateRank(c).toFixed(3),
  )
  tiles.push(
    await sharp(await encodePng(upscaleNearest(c.cell, Math.floor(192 / px))))
      .png()
      .toBuffer(),
  )
}
...
await sharp({
  create: {
    ...
    height: W,
    channels: 4,
    background: { r: 233, g: 226, b: 218, alpha: 1 },
  },
})
