...
const STYLE_ANCHOR = readFileSync(
  process.env.DWELL_REF ?? 'packages/forge/content/reference/style-anchor.png',
)
...
for (const d of DWELLINGS) {
  ...
  writeFileSync(
    `${dest}/report.txt`,
    [...lines.filter((l) => l.startsWith(d.id)), `chosen: ${win.key}`].join('\n'),
  )
  writeFileSync(
    `${dest}/spend.json`,
    JSON.stringify({ asset: `building-${d.id}`, spendUsd: spend[d.id] ?? 0 }, null, 2),
  )
  ...
  rows.push(
    `| ${d.id} | ${d.fp.w}x${d.fp.h} | ${cellPx}x${cellPx} | ${GEN_PX}/${win.plan.factor} (window ${win.plan.window}` +
...
const md = [
  '# the three dwellings of a contemporary San Junipero',
  '',
  ...
  '',
  `spend: ${Object.entries(spend)
    .map(([k, v]) => `${k} $${v.toFixed(4)}`)
    .join(', ')} — total $${budget.total.toFixed(4)} of $${CAP}`,
].join('\n')
...
const stopped = [
  ...(refusedCells.length === 0
    ? []
    : [`${refusedCells.length} dwelling(s) shipped nothing: ${refusedCells.join(', ')}`]),
  ...cls.failures,
]
if (stopped.length > 0)
  throw new Error(
