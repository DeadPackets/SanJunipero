...
const BUILDINGS = [
  ...
  },
  {
    ...
    fp: { w: 1, h: 1 },
    door: false,
    desc: 'a construction scaffolding of honey-wood poles and cream canvas wraps',
  },
  {
    id: 'standing-stone',
    fp: { w: 1, h: 1 },
...
function enumerate(): { units: Unit[]; skipped: string[] } {
  ...
  for (const f of FOUNDERS)
    units.push({
      id: `character:${f.id}`,
      klass: 'character',
      path: join(C5, f.id, 'contact-sheet.png'),
      commission: `${f.desc} — the full contact sheet of poses`,
      expectedFacing:
        'the direction its own row is labelled with; every row must face where it claims',
    })
  ...
  for (const b of BUILDINGS)
    units.push({
      id: `building:${b.id}`,
      klass: 'building',
      path: join(C5, `building-${b.id}`, 'cell.png'),
      commission: b.desc,
      footprint: b.fp,
      expectedFacing: b.door ? 'door to the south-west' : 'no door; light from the north-west',
    })

  if (!existsSync(TILESETS))
    skipped.push(
      `terrain season sheets (${SEASONS.length}): ${TILESETS} does not exist — C10 Task 1 owns it`,
    )
  else
    for (const s of SEASONS) {
      ...
      if (existsSync(p))
        units.push({
          ...
          klass: 'terrain',
          path: p,
          commission: `the ${s} terrain tile sheet`,
        })
...
async function main(): Promise<void> {
  ...
  console.log(
    `vision-audit ${DRY ? 'DRY PLAN' : 'LIVE'} — ${units.length} units, ${units.length} calls, est $${cost.toFixed(4)}`,
  )
  for (const u of units)
    console.log(
      `  ${existsSync(u.path) ? 'ok  ' : 'MISS'} ${u.id.padEnd(28)} ${u.klass.padEnd(10)} ${u.path}`,
    )
  for (const s of skipped) console.log(`  skip ${s}`)
  if (missing.length) {
    console.error(`${missing.length} expected input file(s) missing`)
    process.exit(1)
  }
  if (DRY) {
  ...
  for (const u of units) {
    ...
    writeFileSync(
      join(C13, 'audit', 'verdicts', `${u.id.replace(/:/g, '_')}.json`),
  ...
  const md = [
    ...
    ...['character', 'portrait', 'building', 'terrain']
      .map((k) => ({ k, r: passRates(db, k) }))
      .filter((x) => x.r.n > 0)
      .map(
        ({ k, r }) =>
          `| ${k} | ${r.firstPass.toFixed(3)} | ${r.withinRetries.toFixed(3)} | ${r.blocked.toFixed(3)} | ${r.n} |`,
      ),
    (() => {
      const r = passRates(db)
      return `| ALL | ${r.firstPass.toFixed(3)} | ${r.withinRetries.toFixed(3)} | ${r.blocked.toFixed(3)} | ${r.n} |`
    })(),
    '',
    '## Masters queue (HUMAN EYEBALL — never auto-regenerated)',
    '',
    masters.length
      ? masters.map((r) => `- \`${r.unit.id}\` — ${r.verdict.feedback}`).join('\n')
      : '_empty_',
    '',
    '## Props queue (normal regeneration loop)',
    '',
    props.length
      ? props.map((r) => `- \`${r.unit.id}\` — ${r.verdict.feedback}`).join('\n')
      : '_empty_',
    '',
  ].join('\n')
