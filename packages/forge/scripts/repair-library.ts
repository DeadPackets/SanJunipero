...
const REPAIRS: readonly Repair[] = [
  {
    kind: 'timber',
    srcRaw: 'a2-c1-raw.png',
    defect:
      'density 6.5, proportion 8.5 — salt-and-pepper single-pixel speckle across the ' +
  ...
  },
  {
    ...
    srcRaw: 'a2-c1-raw.png',
    defect:
      'palette 8, density 6.5 — stray single-pixel dots scattered over the seat top and ' +
...
const FILTER = (process.env.ITEMS ?? REPAIRS.map((r) => r.kind).join(','))
  .split(',')
  .map((s) => s.trim())
...
function editPrompt(e: LibraryEntry, note: string): string {
  return (
    `${itemBoilerplate(e)} Reproduce the LAST reference image EXACTLY — the same single object, ` +
...
function deployRepair(e: LibraryEntry, n: number, r: Round, spend: number): void {
  ...
  const prior = JSON.parse(readFileSync(join(dir, 'report.json'), 'utf8')) as Record<
    string,
    unknown
  >
  ...
  const rec = registerLibraryEntry(codex, e, {
    sprite: spritePng,
    icon: iconPng,
    score: Math.min(10, Math.max(1, mean(r.sprite))),
    attempts: n,
    costUsd: spend,
  })
    ...
    JSON.stringify(
      {
        ...
        entries: [
          ...prevEntries.filter((p) => !fresh.entries.some((f) => f.kind === p.kind)),
          ...fresh.entries,
        ],
      },
      null,
...
for (const r of RUN) {
  ...
  if (promote !== undefined) {
    const saved = JSON.parse(
      readFileSync(roundPath(dir, promote, 'verdicts.json'), 'utf8'),
    ) as Round
    if (saved.sprite.overall !== 'pass')
      throw new Error(`${e.kind} r${promote}: sprite gate is red`)
  ...
  for (let strike = 1, n = base + 1; strike <= MAX_ROUNDS && deployed === null; strike++, n++) {
    ...
    console.log(
      `  strike ${n}: sprite ${s.verdict.overall} (${mean(s.verdict).toFixed(1)}) ` +
...
for (const row of rows)
  writeFileSync(join(LIB, row.kind, 'repair.json'), JSON.stringify(row, null, 2))
...
rows.sort(
  (a, b) =>
    REPAIRS.findIndex((r) => r.kind === a.kind) - REPAIRS.findIndex((r) => r.kind === b.kind),
)
...
const md = [
  ...
  '',
  ...REPAIRS.filter((r) => rows.some((x) => x.kind === r.kind)).map(
    (r) => `- \`${r.kind}\` — ${r.defect}\n  - edit: ${r.notes[0]}`,
  ),
  '',
  ...
  '',
  ...rows.flatMap((r) =>
    r.rounds.map((x) => `- \`${r.kind}\` strike ${x.round}: ${x.sprite.feedback}`),
  ),
  '',
].join('\n')
