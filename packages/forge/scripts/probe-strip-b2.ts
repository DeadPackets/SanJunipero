...
async function tryGenerate(
  prompt: string,
  refs: Buffer[],
  sizeParams: Record<string, string>,
): Promise<GenOutcome> {
...
if (existsSync(w0c0Path)) {
  ...
  for (const params of [{ size: '1536x512' }, { aspect_ratio: '3:1' }] as Record<
    string,
    string
  >[]) {
    ...
    if (r.ok) {
      ...
      lines.push(
        `size probe: ${JSON.stringify(params)} ACCEPTED — strip-sw-w0-c0 generated (unguided), total spend $${budget.total.toFixed(3)}`,
      )
      break
    }
...
if (!sizeParams || !firstRaw) {
  lines.push(
    '',
    'RUNG 1 UNAVAILABLE: no wide-canvas request shape accepted — fall through to rung 2',
  )
...
async function candidateRaw(key: string, guided: boolean, margin: boolean): Promise<Buffer | null> {
  ...
  if (existsSync(path)) {
    lines.push(`${key}: reusing cached raw`)
    return readFileSync(path)
  }
  try {
    const r = await tryGenerate(
      stripPrompt(margin),
      guided ? [...REFS, ...GUIDE_REFS] : REFS,
      sizeParams!,
    )
    if (!r.ok) {
      lines.push(`${key}: HTTP ${r.status}: ${r.error}`)
      return null
    }
    writeFileSync(path, r.raw)
    lines.push(
      `${key} generated (${guided ? 'guided' : 'unguided'}${margin ? ', margin' : ''}), total spend $${budget.total.toFixed(3)}`,
    )
    return r.raw
  } catch (e) {
    if (e instanceof BudgetExceededError) {
      lines.push(`${key}: SKIPPED (budget)`)
      return null
    }
    throw e
  }
...
if (results.every(failed0)) {
  lines.push(
    ...results.map(fmtEarly),
    '',
    'w0 candidates failed placement/gates — margin retry (w1)',
  )
  const w1: R[] = []
  for (const [key, guided] of [
    ['strip-sw-w1-c0', false],
    ['strip-sw-w1-c1', true],
  ] as const) {
...
let verdict: string
if (ok.length === 0)
  verdict = 'RUNG1-FAILED: no wide candidate passed slicing+gates — fall through to rung 2'
else if (ok.length === 1) verdict = ok[0]!.guided ? 'guided' : 'unguided'
else {
  const [u, g] = [results.find((r) => !r.guided)!, results.find((r) => r.guided)!]
  verdict =
    Math.abs(u.pitchSpread! - g.pitchSpread!) > 0.1
      ? u.pitchSpread! < g.pitchSpread!
        ? 'unguided'
        : 'guided'
      : g.score > u.score
        ? 'guided'
...
lines.push(
  '',
  `accepted request shape: ${JSON.stringify(sizeParams)}`,
  `VERDICT: ${verdict}`,
