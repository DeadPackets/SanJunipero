...
const OUT =
  process.env.OUT ??
  join(C13, 'reports', ALL ? 'library-master-sheet.png' : `batch-${BATCH}-sheet.png`)
const TITLE =
  process.env.TITLE ?? (ALL ? 'C13 premade library — all 50' : `C13 library batch: ${BATCH}`)

const CELL = 96 // 16 px at 6x and 24 px at 4x both land here
const GAP = 10,
  LABEL_H = 20
const BLOCK_W = CELL * 2 + GAP,
  BLOCK_H = CELL + LABEL_H
const COLS = 5,
  MARGIN = 24,
  HEADER_H = 92,
  GROUP_H = 34,
...
async function main(): Promise<void> {
  ...
  for (const p of placed) {
    if (p.sprite)
      compositeOver(
        canvas,
        p.sprite,
        p.x + ((CELL - p.sprite.width) >> 1),
        p.y + ((CELL - p.sprite.height) >> 1),
      )
    if (p.icon)
      compositeOver(
        canvas,
        p.icon,
        p.x + CELL + GAP + ((CELL - p.icon.width) >> 1),
        p.y + ((CELL - p.icon.height) >> 1),
      )
  }
  ...
  const out = await sharp(await encodePng(canvas))
    .composite([{ input: overlay, top: 0, left: 0 }])
    .png()
    .toBuffer()
  writeFileSync(OUT, out)
  console.log(
    `wrote ${OUT} (${width}x${height}, ${placed.length} entries, ${placed.filter((p) => p.sprite).length} with art)`,
  )
}
