...
for (const d of [`${DURABLE}/cells`, `${DURABLE}/raws`, `${DURABLE}/gifs`, CACHE])
...
async function candidate(
  ...
  if (existsSync(rawPath)) {
    ...
    const base = extraRefs.length
      ? [...REFS.slice(0, -1), ...extraRefs, REFS[REFS.length - 1]!]
      : REFS
    raw = await generate(prompt, guided ? [...base, ...GUIDE_REFS] : base, size)
    writeFileSync(rawPath, raw)
    console.log(
      `  ${key}: generated (${guided ? 'guided' : 'unguided'}), total spend $${budget.total.toFixed(3)}`,
    )
  }
  let v = scores[key]
  if (!v) {
...
async function attemptStrip(
  f: Facing,
  attempt: number,
): Promise<{ ok: boolean; state: StripState }> {
  ...
  for (let i = 0; i < 2; i++) {
    ...
    try {
      cands.push(
        await candidate(`strip-${f}-a${attempt}-c${i}`, stripPrompt(f), guided, '1536x512'),
      )
    } catch (e) {
  ...
  for (const cand of cands) {
    ...
    try {
      strip = processStrip(chromaKey(await decodePng(cand.raw)))
    } catch (e) {
      reportLines.push(
        `${cand.key} (${cand.guided ? 'guided' : 'unguided'}) score=${cand.score} — process FAILED: ${String(e)}`,
      )
      continue
    }
    const failures = withinFacingFailures(f, strip.cells, PROVISIONAL_MEDIAN)
    reportLines.push(
      `${cand.key} (${cand.guided ? 'guided' : 'unguided'}) score=${cand.score} ` +
...
for (const f of FACINGS) {
  ...
  for (let attempt = 0; attempt <= 2; attempt++) {
    ...
    try {
      r = await attemptStrip(f, attempt)
    } catch (e) {
      if (e instanceof OutOfBudget && last) {
        console.log(`budget exhausted; keeping ${f} attempt ${last.retries}`)
        break
      }
      throw e
    }
    ...
    if (r.ok) break
    console.log(
      `  ${f} attempt ${attempt} failed gates: ${r.state.failures.map((x) => `${x.gate} ${x.a}~${x.b} ${x.value.toFixed(3)}<${x.limit.toFixed(3)}`).join('; ')}`,
    )
  }
  strips.set(f, last!)
}
...
async function attemptSleep(
  f: Facing,
  attempt: number,
  seed?: Buffer,
): Promise<{ ok: boolean; state: SleepState }> {
  const cands: Candidate[] = []
  for (let i = 0; i < 2; i++) {
    ...
    try {
      cands.push(await candidate(key, sleepPrompt(f), guided, '512x512', seed ? [seed] : []))
    } catch (e) {
      if (e instanceof BudgetExceededError) throw new OutOfBudget(e.message)
      throw e
    }
...
for (const f of FACINGS) {
  ...
  for (let attempt = 0; attempt <= 2; attempt++) {
    ...
    try {
      r = await attemptSleep(f, attempt)
    } catch (e) {
      if (e instanceof OutOfBudget && last) {
        console.log(`budget exhausted; keeping ${f}/sleep attempt ${last.retries}`)
        break
      }
      throw e
    }
    ...
    if (r.ok) break
    console.log(
      `  ${f}/sleep attempt ${attempt} failed: ${r.state.failures.map((x) => x.gate).join(', ')}`,
    )
  }
    ...
    if (sw && sw.failures.length === 0) {
      try {
        ...
        if (r.ok) last = r.state
          ...
          console.log(
            `  ${f}/sleep seeded attempt failed: ${r.state.failures.map((x) => x.gate).join(', ')} — stays BLOCKED`,
          )
      } catch (e) {
        if (e instanceof OutOfBudget)
          console.log(`budget exhausted; ${f}/sleep seeded attempt skipped`)
        else throw e
      }
    }
  }
...
function crossFailures(median: number): { pose: PoseV2; failures: GateFailure[] }[] {
  return POSES_V2.map((p) => ({
    pose: p,
    failures: crossFacingDupeGate(
      FACINGS.map((f) => ({ label: label(f, p), img: cellOf(f, p) })),
      median,
    ),
...
crossLoop: for (let round = 1; round <= 2; round++) {
  ...
  for (const { pose, failures } of failing)
    for (const fnd of failures) {
      ...
      if (pose === 'sleep') {
        ...
        try {
          sleeps.set(f, (await attemptSleep(f, st.retries + 1)).state)
        } catch (e) {
          if (e instanceof OutOfBudget) break crossLoop
          throw e
        }
      } else {
        ...
        try {
          strips.set(f, (await attemptStrip(f, st.retries + 1)).state)
        } catch (e) {
          if (e instanceof OutOfBudget) break crossLoop
          throw e
        }
...
for (const f of FACINGS) {
  for (const p of POSES_V2)
    writeFileSync(`${DURABLE}/cells/${file(f, p)}`, await encodePng(cellOf(f, p)))
...
for (const f of FACINGS) {
  ...
  const gif = await sharp(Buffer.from(stacked.buffer), {
    ...
    .gif({ delay: frames.map(() => 125), loop: 0 })
    .toBuffer()
...
const report = [
  ...
  '',
  '== candidates (guided/unguided, judge score, pitch, gate outcome) ==',
  ...reportLines,
  '',
  '== chosen strips ==',
  ...FACINGS.map((f) => {
  ...
  '',
  '== straight distance matrix (24x24) ==',
  distanceMatrix(cells24, false),
  '',
  '== mirrored distance matrix (24x24) ==',
  distanceMatrix(cells24, true),
  '',
