...
async function generateCell(f: Facing, p: Pose, n: number, refs: Buffer[], doubleFacing = false, doublePose = false): Promise<Attempt> {
  ...
  for (const c of cands) {
    ...
    console.log(`  ${label(f, p)} candidate (${c.model}): score=${v.score} $${c.costUsd.toFixed(3)} — ${v.notes}`)
    if (!best || v.score > best.score) best = { raw: c.png, cell: await postProcessCell(c.png), score: v.score, notes: v.notes }
  }
  return best!
}
...
function collectFindings() {
  const rows = POSES.flatMap(p => duplicateReport(
    FACINGS.map(f => ({ label: label(f, p), img: cells.get(label(f, p))!.current.cell })), STRAIGHT_DUPE, MIRROR_DUPE))
  const cols = FACINGS.flatMap(f => duplicateReport(
    POSES.map(p => ({ label: label(f, p), img: cells.get(label(f, p))!.current.cell })), STRAIGHT_DUPE, MIRROR_DUPE))
  return { rows, cols }
}
