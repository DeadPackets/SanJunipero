...
const CAST_FILTER = (process.env.CAST ?? PORTRAIT_CAST.map(c => c.id).join(',')).split(',').map(s => s.trim())
...
async function generate(asset: string, prompt: string, refs: Buffer[]): Promise<Buffer> {
  if (budget.total + RESERVE > CAP) throw new OutOfBudget(`reserve $${RESERVE} exceeds remaining cap ($${budget.total.toFixed(3)} of $${CAP} spent)`)
...
async function runCharacter(m: PortraitMember): Promise<void> {
  ...
  for (const key of (process.env.REJECT ?? '').split(',').map(s => s.trim()).filter(Boolean)) {
  ...
  const report: string[] = [`== ${m.id} portrait run (B3 recipe, identity anchor = adopted master) ==`]
  const neutrals: Candidate[] = []
  for (let i = 0; i < 3; i++) {
    const c = await candidate(`neutral-c${i}`, portraitPrompt(m.desc, 'calm, friendly, neutral resting face'), baseRefs)
    report.push(c ? `neutral-c${i}: score=${c.score} — ${c.notes}` : `neutral-c${i}: post-process FAILED`)
    if (c) neutrals.push(c)
  }
  if (neutrals.length === 0) throw new Error(`${m.id}: every neutral candidate failed post-processing`)
  ...
  const forced = process.env.NEUTRAL_PICK !== undefined && RUN_CAST.length === 1
    ? neutrals.find(c => c.key === `neutral-c${process.env.NEUTRAL_PICK}`) : undefined
  const neutralPick = forced ?? neutrals.reduce((a, c) => (c.score > a.score ? c : a))
  report.push(`neutral RUNNER PICK: ${neutralPick.key} (${forced ? 'runner-forced for framing consistency' : 'highest judge score'}; human ratification pending, B1/B3 precedent)`)
  ...
  for (const expr of EXPRESSIONS) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const c = await candidate(`${expr}-c${i}`, portraitPrompt(m.desc, EXPRESSION_CLAUSES[expr]), exprRefs)
      if (!c) { report.push(`${expr}-c${i}: post-process FAILED`); continue }
    ...
    if (cands.length === 0) throw new Error(`${m.id}/${expr}: every candidate failed post-processing`)
    ...
    const pickKey = (process.env.EXPR_PICK ?? '').split(',').map(s => s.trim())
      .find(s => s.startsWith(`${expr}:`))?.split(':')[1]
    const forcedExpr = pickKey ? cands.find(x => x.c.key === `${expr}-${pickKey}` && x.problems.length === 0) : undefined
    const best = forcedExpr ?? cands.reduce((a, x) => {
      if ((x.problems.length === 0) !== (a.problems.length === 0)) return x.problems.length === 0 ? x : a
      return x.c.score > a.c.score ? x : a
    })
    if (forcedExpr) report.push(`${expr}: eyeball pick ${forcedExpr.c.key} (consistency PASS; judge advisory)`)
    if (best.problems.length > 0) { flagged++; report.push(`${expr}: ships FLAGGED (${best.c.key}: ${best.problems.join('; ')})`) }
  ...
  writeFileSync(`${DIR}/contact-sheet.png`, await encodePng(assembleGrid([row], SHIP * 4, SHIP * 4)))

  report.push('', flagged === 0 ? 'all expressions PASS consistency' : `** ${flagged} expression(s) ship FLAGGED **`,
  ...
  const prevSpend = (() => {
    try { return (JSON.parse(readFileSync(`${DIR}/spend.json`, 'utf8')) as { spendUsd?: number }).spendUsd ?? 0 } catch { return 0 }
  })()
  writeFileSync(`${DIR}/spend.json`, JSON.stringify({ asset: `${m.id}-portraits`, spendUsd: prevSpend + (assetSpend[m.id] ?? 0) }, null, 2))
  globalReport.push(`${m.id}: 7/7 shipped, ${flagged === 0 ? 'all gates PASS' : `${flagged} FLAGGED`}, spend $${(assetSpend[m.id] ?? 0).toFixed(3)}`)
  console.log(report.join('\n'))
}

for (const m of RUN_CAST) {
  if (budget.total + START_HEADROOM > CAP) {
    globalReport.push(`${m.id}: NOT STARTED — headroom $${(CAP - budget.total).toFixed(3)} < $${START_HEADROOM} (whole-set rule)`)
