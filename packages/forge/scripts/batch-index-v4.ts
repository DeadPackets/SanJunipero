...
function label(text: string): Promise<Buffer> {
  const svg = `<svg width="${W}" height="${LABEL_H}" xmlns="http://www.w3.org/2000/svg">` +
...
async function fitWidth(path: string, width: number): Promise<Buffer> {
  return sharp(readFileSync(path)).resize({ width, kernel: 'lanczos3' })
...
await add(await label('SAN JUNIPERO ASSET PRODUCTION — standard v3 (hi-res mirror pipeline) — HUMAN EYEBALL GATE'))
...
for (const [name, path] of CHARS) {
  if (!existsSync(path)) { console.log(`SKIP ${name}: ${path} missing`); continue }
  const note = name === 'omar' ? 'omar — APPROVED REFERENCE (not regenerated this run)' :
...
await add(await label('buildings — storehouse 2x2 | wagon 1x2 | shed 1x1 | scaffolding 1x1 | standing stone 1x1 (hut = approved anchor cottage)'))
{
  const H = 340
  const thumbs: { buf: Buffer; w: number }[] = []
  for (const b of BUILDINGS) {
    const p = `${PRODUCTION}/building-${b}/preview-4x.png`
    if (!existsSync(p)) { console.log(`SKIP building ${b}`); continue }
    const buf = await sharp(readFileSync(p)).resize({ height: H, kernel: 'lanczos3' })
      .flatten({ background: BG }).png().toBuffer()
    const meta = await sharp(buf).metadata()
  ...
  for (const t of thumbs) { comps.push({ input: t.buf, left: Math.min(x, W - t.w), top: 0 }); x += t.w + gap }
...
const comps: sharp.OverlayOptions[] = []
let y = 0
for (const p of parts) { comps.push({ input: p.buf, left: 0, top: y }); y += p.h }
