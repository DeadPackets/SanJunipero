...
const ONLY = (process.env.STRUCT_ONLY ?? '').split(',').map((s) => s.trim()).filter(Boolean)
...
async function paletteSwatch(): Promise<Buffer> {
  const cols = 8, rows = 5, sw = 64
  const w = cols * sw, h = rows * sw
  const data = new Uint8ClampedArray(w * h * 4)
  const rgb = paletteRgb(MASTER_PALETTE)
  for (let i = 0; i < rgb.length; i++) {
    const [r, g, b] = rgb[i]!
    const cx = (i % cols) * sw, cy = Math.floor(i / cols) * sw
...
const FACING_CLAUSE: Record<StructureFacing, string> = {
  sw: 'ORIENTATION: the FRONT of the object — its door, its opening, its head end, whatever '
  ...
  se: 'ORIENTATION: the object is turned NINETY DEGREES from the usual view. The FRONT of the '
...
const SUBJECTS: readonly Subject[] = [
  {
    id: 'well', kind: 'well', fp: { w: 1, h: 1 }, cells: 'one', clause: 'symmetric',
    desc:
  ...
  {
    id: 'fire-pit', kind: 'fire_pit', fp: { w: 1, h: 1 }, cells: 'one', clause: 'symmetric',
    desc:
  ...
  {
    id: 'standing-stone', kind: 'standing_stone', fp: { w: 1, h: 1 }, cells: 'one', clause: 'symmetric',
    desc:
  ...
  {
    id: 'grave', kind: 'grave', fp: { w: 1, h: 1 }, cells: 'one', clause: 'symmetric',
    desc:
  ...
  {
    id: 'scaffolding', kind: 'scaffolding', fp: { w: 1, h: 1 }, cells: 'one', clause: 'symmetric',
    desc:
  ...
  {
    id: 'wagon', kind: 'wagon', fp: { w: 1, h: 2 }, cells: 'two', clause: 'oriented',
    desc:
  ...
  {
    id: 'bridge', kind: 'bridge', fp: { w: 1, h: 2 }, cells: 'one', clause: 'oriented',
    desc:
...
function prompt(s: Subject, facing: StructureFacing): string {
  return `${STYLE_PROMPT} A single free-standing object sprite. `
    + `Subject: ${s.desc}. `
...
for (const s of SUBJECTS) {
  ...
  for (const facing of facingsOf(s)) {
    ...
    for (let i = 0; i < MAX_ATTEMPTS; i++) {
      ...
      try {
        ...
        const fails = [
          ...
          ...nativeDensityGate({ name: label, canvas: { w: cellPx, h: cellPx }, footprint: s.fp, tile: TOWN_TILE }).failures,
        ]
        const refused = REJECTED.has(key)
        if (!refused) cands.push({ key, cell: r.cell, plan: r.plan, fails, subject })
        const msg = `${label}: ${key} subject ${subject}px, factor ${r.plan.factor}, window ${r.plan.window}, `
    ...
    writeFileSync(`${dir}/manifest.json`, `${JSON.stringify({
      version: 'v4-hires-building', kind: facingKind(s.kind, facing), footprint: s.fp,
      cell: cellAnchor(win.cell),
    }, null, 2)}\n`)

    const density = spriteDensity({ canvas: { w: cellPx, h: cellPx }, footprint: s.fp, tile: TOWN_TILE })
...
const md = ['# the eight bare kinds, in ten cells', '',
