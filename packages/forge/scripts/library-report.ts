...
type Row = {
  ...
  attempts: number
  spendUsd: number
  sprite: VisionVerdict | null
  icon: VisionVerdict | null
}
...
for (const e of LIBRARY) {
  ...
  rows.push({
    ...
    sprite: r.spriteVerdicts.at(-1) ?? null,
    icon: r.iconVerdicts.at(-1) ?? null,
  })
}
...
const perCriterion = CRITERIA.map((c) => {
  const vs = rows
    .flatMap((r) => [r.sprite, r.icon].filter((v): v is VisionVerdict => v !== null))
    .filter((v) => live(v, c))
  return { c, n: vs.length, held: vs.filter((v) => holds(v, c)).length }
})
const allVerdicts = rows.flatMap((r) =>
  [r.sprite, r.icon].filter((v): v is VisionVerdict => v !== null),
)
const criteriaHeld = allVerdicts.reduce((s, v) => s + CRITERIA.filter((c) => holds(v, c)).length, 0)
const criteriaTotal = allVerdicts.reduce(
  (s, v) => s + CRITERIA.filter((c) => asked(v, c)).length,
  0,
)
...
const md = [
  ...
  ...LIBRARY_CATEGORIES.map((c) => {
    ...
    return (
      `| ${c} | ${rs.length} | ${rs.filter((r) => r.status === 'pass').length} | ` +
  ...
  ...rows.map(
    (r) =>
      `| \`${r.kind}\` | ${r.category} | **${r.status}** | **${r.icon?.overall ?? '—'}** | ` +
...
console.log(
  `wrote library-report.md — sprite ${spritePass}/50, icon ${iconPass}/50, ` +
