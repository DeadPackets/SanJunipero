...
const BANDS = { hair: [0.06, 0.3], skin: [0.36, 0.62] } as const

function bandTones(img: RawImage, band: readonly [number, number]): number {
  const y0 = Math.round(img.height * band[0]),
    y1 = Math.round(img.height * band[1])
...
for (const c of CASTS) {
  if (!existsSync(c.dir)) {
    console.log(`${c.id}: no portraits at ${c.dir}`)
    continue
  }
  const dest = join(OUT, c.id)
  mkdirSync(dest, { recursive: true })
  for (const f of readdirSync(c.dir)
    .filter((n) => n.endsWith('.png') && !n.includes('-4x'))
    .sort()) {
    ...
    rows.push({
      who: c.id,
      expr: f.replace('.png', ''),
      hairBefore: bandTones(shipped, BANDS.hair),
      hairMaster: bandTones(master, BANDS.hair),
      hairAfter: bandTones(ramped, BANDS.hair),
      skinBefore: bandTones(shipped, BANDS.skin),
      skinMaster: bandTones(master, BANDS.skin),
    ...
    if (!g.ok) {
      refused.push(`${c.id}/${f}: ${g.failures.join('; ')}`)
      continue
    }
    writeFileSync(join(dest, f), await encodePng(ramped))
    writeFileSync(
      join(dest, f.replace('.png', '-4x.png')),
      await encodePng(upscaleNearest(ramped, 4)),
    )
...
const md = [
  ...
  ...rows.map(
    (r) =>
      `| ${r.who} | ${r.expr} | ${r.hairBefore} → ${r.hairMaster} → **${r.hairAfter}** ` +
...
md.push(
  '',
  `**mean hair** ${mean((r) => r.hairBefore)} → ${mean((r) => r.hairMaster)} → **${mean((r) => r.hairAfter)}**` +
