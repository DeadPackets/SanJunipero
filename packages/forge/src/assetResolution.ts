...
const row = (klass: string, native: Size, genPx: number, todayPx: number, note: string): ResolutionRow =>
  ({ klass, genPx, todayPx, note, ...resolveScale(native, genPx) })
...
export function resolutionTable(): ResolutionRow[] {
  ...
  return [
    row('item', { w: 128, h: 128 }, g.item, 24, '1x1 furnishing or held item on the 128x64 interior tile'),
    row('item-1x2', { w: 192, h: 192 }, g.item, 24, 'bed, rug, bench, loom — a 1x2 footprint spans 192 px'),
    row('item-2x2', { w: 256, h: 256 }, g.item, 24, 'the largest furnishing a 512 generation can hold'),
    row('icon', { w: 64, h: 64 }, g.icon, 24, 'inventory and roster icon, resampled from the same generation'),
    row('interior-tile', { w: 128, h: 64 }, g['interior-tile'], 32, 'one interior floor tile at 4x town scale'),
    row('terrain', { w: 256, h: 256 }, g.terrain, 128, 'a seamless ground material, sampled through the tile window'),
    ...
    row('building', { w: 256, h: 256 }, g.building, 128, 'a 2x2 dwelling; the renderer then halves it exactly'),
    row('structure', { w: 256, h: 256 }, g.structure, 128, 'wagon, scaffolding, standing stone — same bar as buildings'),
    row('character-cell', { w: 256, h: 256 }, g['character-cell'], 96, 'one pose cell; today these are UPSCALED to ~845'),
    row('portrait', { w: 256, h: 256 }, g.portrait, 128, 'bust portrait, one expression per generation'),
  ]
}
