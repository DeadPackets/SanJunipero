...
import {
  CRITERIA, VisionCriterionSchema, VisionVerdictSchema, NA_CRITERIA_BY_CLASS, NA_CRITERION,
  deriveOverall, type VisionCriteria, type VisionVerdict,
} from './verdict.js'
...
export type VisionJudgeArgs = {
  assetId: string; klass: string; sprite: RawImage; commission: string
  footprint?: Footprint; expectedFacing?: string
  attempt?: number      // ruling R-1: the gate passes the real attempt so the recorded overall is true
}
export type VisionJudgeFn = (args: VisionJudgeArgs) => Promise<{ verdict: VisionVerdict; costUsd: number }>
...
export function makeVisionJudge(opts: {
  ...
  return async a => {
    ...
    const text = buildRubricPrompt({
      klass: a.klass, commission: a.commission, footprint: a.footprint,
      expectedFacing: a.expectedFacing, naFor,
    })
    const { object, providerMetadata } = await gen({
      ...
      messages: [{
        ...
        content: [
          ...
          ...opts.refs.map(image => ({ type: 'image' as const, image, mediaType: 'image/png' as const })),
          { type: 'image' as const, image: await encodePng(paletteCard()), mediaType: 'image/png' as const },
          { type: 'image' as const, image: await encodePng(checkerCard(a.sprite)), mediaType: 'image/png' as const },
        ],
      }],
    })
    ...
    const verdict = VisionVerdictSchema.parse({
      assetId: a.assetId, model: modelName, rubricVersion: RUBRIC_VERSION, criteria, feedback,
      overall: deriveOverall(criteria, {
        minScore: config.visionQa.minScore, attempt: a.attempt ?? 1,
        maxRetries: config.visionQa.maxRetries, naFor,
      }),
    })
