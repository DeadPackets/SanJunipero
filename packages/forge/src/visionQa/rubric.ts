...
export const CHECKER_PX = 8, CARD_PAD = 8, SWATCH_PX = 12
export const CHECKER_LIGHT = '#E9E2DA', CHECKER_DARK = '#CFC6BC'   // palette greys, never magenta

const PALETTE_COLS = 8, PALETTE_ROWS = 5, CHECKER_MARGIN = 16

function rgb(h: string): [number, number, number] {
  return [parseInt(h.slice(1, 3), 16), parseInt(h.slice(3, 5), 16), parseInt(h.slice(5, 7), 16)]
}

function fill(img: RawImage, x0: number, y0: number, w: number, h: number, c: [number, number, number]): void {
...
export function paletteCard(): RawImage {
  ...
  MASTER_PALETTE.forEach((hex, i) => {
    const col = i % PALETTE_COLS, row = (i / PALETTE_COLS) | 0
    fill(img, CARD_PAD + col * SWATCH_PX, CARD_PAD + row * SWATCH_PX, SWATCH_PX, SWATCH_PX, rgb(hex))
  })
  return img
}
...
function criterionAsk(k: Criterion, a: { klass: string; expectedFacing?: string }): string {
  ...
  switch (k) {
    case 'palette': return 'palette — every opaque colour is a member of the palette card shown above. No colour outside it, no gradient bands, no soft anti-aliased edges.'
    case 'singleFigure': return 'singleFigure — exactly one subject in the frame. No duplicate copy, no inset thumbnail, no second view, no background scene. Binary: fail outright if more than one subject is present.'
    case 'transparency': return 'transparency — the background is entirely clear. No magenta residue, no coloured halo, no grey fringe hugging the subject. Binary: fail outright on any residue. The checker behind the artwork makes residue visible.'
    case 'proportion': return pitch !== undefined
...
export function buildRubricPrompt(args: {
  klass: string; commission: string; footprint?: Footprint; expectedFacing?: string
  ...
  if (naNames.length) {
    lines.push('', `Not judged for this class: ${naNames.join(', ')} — ` +
