...
export type VisionGateArgs = {
  assetId: string; klass: string; commission: string
  regenerate: (prompt: string, attempt: number) => Promise<{ sprite: RawImage; costUsd: number; model: string }>
  basePrompt: { boilerplate: string; commissionText: string }
  judge: VisionJudgeFn; ledger: SpendLedger; config: ForgeConfig
  footprint?: Footprint; expectedFacing?: string
}

export type VisionGateResult = {
  status: 'pass' | 'blocked'; sprite: RawImage; verdicts: VisionVerdict[]
  attempts: number; spendUsd: number; overQaCap: boolean
}

const totalScore = (v: VisionVerdict): number => CRITERIA.reduce((s, k) => s + v.criteria[k].score, 0)
...
export async function runVisionGate(a: VisionGateArgs): Promise<VisionGateResult> {
  ...
  for (;;) {
    ...
    if (!a.config.visionQa.enabled)
      return {
        status: 'pass', sprite: gen.sprite, verdicts: [], attempts: attempt,
        spendUsd: a.ledger.totalFor(a.assetId), overQaCap: false,
      }

    const { verdict, costUsd } = await a.judge({
      assetId: a.assetId, klass: a.klass, sprite: gen.sprite, commission: a.commission,
      footprint: a.footprint, expectedFacing: a.expectedFacing, attempt,
    })
