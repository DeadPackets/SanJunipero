...
export const JudgeVerdict = z
  .object({ score: z.number().min(1).max(10), notes: z.string() })
  .strict()
export type JudgeFn = (candidatePng: Buffer) => Promise<z.infer<typeof JudgeVerdict>>
export type GenerateFn = (args: {
  model: unknown
  schema: typeof JudgeVerdict
  messages: unknown[]
...
export function makeVlmJudge(opts: {
  apiKey: string
  refSheets: Buffer[]
  model?: string
