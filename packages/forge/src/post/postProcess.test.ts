...
async function fakeGeneration(): Promise<Buffer> {
  ...
  return sharp({
    ...
    .composite([{ input: red, left: 16, top: 16 }])
    .png()
    .toBuffer()
}
...
describe('postProcess', () => {
  it('building: keys magenta, downscales NEAREST, quantizes, outlines', async () => {
    const out = await decodePng(
      await postProcess(await fakeGeneration(), 'building', { w: 32, h: 32 }),
    )
  ...
  it('non-square keyed target (crop, 128x32): sprite keeps its proportions', async () => {
    ...
    const gen = await sharp({
      ...
      .composite([{ input: red, left: 224, top: 224 }])
      .png()
      .toBuffer()
