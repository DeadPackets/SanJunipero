...
function bigPixelArt(w: number, h: number, block: number, cells: (x: number, y: number) => [number, number, number] | null): RawImage {
...
const plus = (cx: number, cy: number, r: number) =>
  bigPixelArt(256, 256, 16, (x, y) => {
    const on = (Math.abs(x - cx) <= 1 && Math.abs(y - cy) <= r) || (Math.abs(y - cy) <= 1 && Math.abs(x - cx) <= r)
...
describe('toSpriteCell', () => {
  ...
  it('fits a wide figure by its long side instead of overflowing', () => {
    const wide = bigPixelArt(256, 256, 16, (x, y) => (y === 8 && x >= 1 && x <= 14 ? [176, 124, 78] : null))
  ...
  it('drops isolated specks left by the lattice resample', () => {
    ...
    let cornerOpaque = 0
    for (let y = 0; y < 4; y++) for (let x = 20; x < 24; x++)
      if (cell.data[(y * 24 + x) * 4 + 3] !== 0) cornerOpaque++
  ...
  const dense = bigPixelArt(256, 256, 16, (x, y) =>
    x >= 2 && x <= 12 && y >= 2 && y <= 12
      ? ((x + y) % 2 ? [242, 214, 175] : [176, 124, 78]) : null)
  ...
  it('ranks a clean single silhouette above a subject with floating debris', () => {
    ...
    expect(candidateRank(clean)).toBeLessThan(candidateRank({ ...clean, opaqueFrac: clean.opaqueFrac / 2 }))
