...
export function registerLibraryEntry(
  codex: AssetCodex,
  e: LibraryEntry,
  args: {
    sprite: Buffer
...
// Largest integer divisor that fits the sprite inside iconPx, then centred. Nearest sampling keeps
// alpha and quantized colours untouched, and k=1 makes an already-small sprite a no-op.
export function deriveIcon(sprite: RawImage, iconPx: number): RawImage {
  ...
  const w = Math.max(1, Math.ceil(sprite.width / k)),
    h = Math.max(1, Math.ceil(sprite.height / k))
  ...
  const ox = (iconPx - small.width) >> 1,
    oy = (iconPx - small.height) >> 1
  for (let y = 0; y < Math.min(small.height, iconPx); y++) {
    const src = y * small.width * 4
    data.set(
      small.data.subarray(src, src + Math.min(small.width, iconPx) * 4),
      ((y + oy) * iconPx + ox) * 4,
    )
  }
