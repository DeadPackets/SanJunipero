...
const CATEGORY_HINT: Record<LibraryCategory, string> = {
  ...
  furniture:
    'A single piece of furniture in 2:1 dimetric projection, seen from the fixed camera above and to the left.',
}
...
export function itemCommission(e: LibraryEntry): string {
  return (
    `Subject: ${e.desc}. One object only, centred, filling the frame, nothing else in view.` +
...
export function planBatch(batch: string, opts: { candidates?: number } = {}): PlannedItem[] {
  ...
  return LIBRARY.filter((e) => e.category === BATCH_CATEGORY[b]).map((entry) => {
    ...
    return {
      ...
      boilerplate,
      commissionText,
      prompt: `${boilerplate} ${commissionText}`,
      candidates,
    }
  })
}
...
export function estimateBatchCost(items: PlannedItem[]): number {
  return items.reduce(
    (s, i) => s + i.candidates * EST_COST_PER_IMAGE + 2 * EST_COST_PER_VISION_CALL,
    0,
  )
}
