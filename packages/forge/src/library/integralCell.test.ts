...
describe('integralSpriteCell', () => {
  ...
  it('lands on the cell by an exact whole-number division of its crop', async () => {
    ...
    expect(integerScaleGate(r.crop, { w: 128, h: 128 })).toMatchObject({ ok: true, factor: r.factor })
  })
  ...
  const cream = [0xff, 0xf6, 0xe9, 255], dark = [0x17, 0x14, 0x20, 255]
  ...
  it('takes the majority colour of each block, not a corner sample', () => {
    ...
    data.set(dark, 0)
    const r = integralSpriteCell({ width: 4, height: 4, data }, 2, { minFactor: 2, keepEmpty: true })
  ...
  it('a block that is mostly clear comes back clear, and alpha stays binary', () => {
    ...
    for (const [x, y] of [[3, 0], [2, 1], [3, 1]] as const) data.set([0, 0, 0, 0], (y * 8 + x) * 4)
    const r = integralSpriteCell({ width: 8, height: 8, data }, 4, { minFactor: 2, keepEmpty: true })
    expect(r.cell.data[3]).toBe(255)          // block (0,0): 4 of 4 opaque
    expect(r.cell.data[4 + 3]).toBe(0)        // block (1,0): 1 of 4 opaque
    expect([...r.cell.data].filter((_, i) => i % 4 === 3).every(a => a === 0 || a === 255)).toBe(true)
  })
})
