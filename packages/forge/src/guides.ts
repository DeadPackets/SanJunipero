...
export function renderStripFrameGuide(): RawImage {
  ...
  for (let slot = 0; slot < STRIP_GUIDE_SLOTS; slot++) {
    const x0 = slot * STRIP_GUIDE_H + STRIP_GUIDE_INSET,
      x1 = (slot + 1) * STRIP_GUIDE_H - 1 - STRIP_GUIDE_INSET
    const y0 = STRIP_GUIDE_INSET,
      y1 = h - 1 - STRIP_GUIDE_INSET
    for (let y = y0; y <= y1; y++)
      for (let x = x0; x <= x1; x++) {
        const onEdge =
          x - x0 < STRIP_GUIDE_LINE ||
          x1 - x < STRIP_GUIDE_LINE ||
          y - y0 < STRIP_GUIDE_LINE ||
          y1 - y < STRIP_GUIDE_LINE
