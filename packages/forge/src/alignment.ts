...
export function toTargetCell(
  img: RawImage,
  man: { footprint: Footprint; cell: { w: number; h: number; feetX: number; feetY: number } },
...
export function validateBuildingAlignment(
  ...
  ok: boolean
  failures: string[]
  measured: { bottomY: number; baseLeft: number; baseRight: number }
} {
  ...
  if (bottomY < 0)
    return {
      ok: false,
      failures: ['no opaque pixels'],
      measured: { bottomY: -1, baseLeft: -1, baseRight: -1 },
    }
  ...
  if (delta > cfg.feetTolerancePx)
    failures.push(
      `feet line: base bottom y=${bottomY} is ${delta}px from the near vertex y=${d.nearVertexY} (tolerance ${cfg.feetTolerancePx}px)`,
    )
  if (belowCount > 0)
  ...
  let baseLeft = Number.POSITIVE_INFINITY,
    baseRight = Number.NEGATIVE_INFINITY
  ...
  if (baseRight < baseLeft) {
    ...
    if (baseLeft < d.leftX - tol || baseRight > d.rightX + tol)
      failures.push(
        `base fit: base spans x=${baseLeft}..${baseRight}, outside the diamond ${d.leftX}..${d.rightX} (tolerance ${tol}px)`,
      )
  }
...
export function testGridRender(img: RawImage, fp: Footprint): RawImage {
  ...
  const ox = GRID_MARGIN,
    oy = GRID_MARGIN
  const vx = d.centerX + ox,
    vy = d.nearVertexY + oy // the near vertex, lattice origin

  // two families of tile-lattice hairlines, +x = (+16,+8) and +y = (-16,+8)
  for (let i = -GRID_SPAN; i <= GRID_SPAN; i++) {
    const ax = vx + (i * TILE_W) / 2,
      ay = vy + (i * TILE_H) / 2
    line(
      out,
      ax - (GRID_SPAN * TILE_W) / 2,
      ay + (GRID_SPAN * TILE_H) / 2,
      ax + (GRID_SPAN * TILE_W) / 2,
      ay - (GRID_SPAN * TILE_H) / 2,
      HEX_ROAD_EDGE,
    )
    const bx = vx - (i * TILE_W) / 2,
      by = vy + (i * TILE_H) / 2
    line(
      out,
      bx - (GRID_SPAN * TILE_W) / 2,
      by - (GRID_SPAN * TILE_H) / 2,
      bx + (GRID_SPAN * TILE_W) / 2,
      by + (GRID_SPAN * TILE_H) / 2,
      HEX_ROAD_EDGE,
    )
