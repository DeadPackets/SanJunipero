...
describe('emotes', () => {
  ...
  it('is deterministic', () => {
    for (const kind of EMOTE_KINDS)
      expect(renderEmote(kind).data).toEqual(renderEmote(kind).data)
  })
  it('every opaque pixel uses a palette color', () => {
    const palette = new Set(Object.values(EMOTE_PALETTE).map(([r, g, b]) => (r << 16) | (g << 8) | b))
    for (const kind of EMOTE_KINDS) {
      const img = renderEmote(kind)
      for (let i = 0; i < img.data.length; i += 4) {
        if (img.data[i + 3] === 0) continue
        expect(palette.has((img.data[i]! << 16) | (img.data[i + 1]! << 8) | img.data[i + 2]!)).toBe(true)
      }
    }
  })
  it('all 12 glyphs are pairwise distinct', () => {
    for (let i = 0; i < EMOTE_KINDS.length; i++) for (let j = i + 1; j < EMOTE_KINDS.length; j++) {
