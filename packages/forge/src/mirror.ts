...
export const CELL_NAMES_V4: readonly string[] = POSES_V2.flatMap((p) =>
  FACINGS.map((f) => `${p}-${f}`),
)
...
export function stanceGate(
  facing: string,
  idle: RawImage,
  contacts: { label: string; img: RawImage }[],
  min = STANCE_MIN_RATIO,
): GateFailure[] {
