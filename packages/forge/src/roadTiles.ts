...
export const TILE_W = 32,
  TILE_H = 16
export const ROAD_BASE = 0xd4bc9e,
  ROAD_EDGE = 0xb89d7e,
  ROAD_GRIT = 0xe8d5bc // MASTER_PALETTE members
...
export function paintRoadAutotile(key: RoadAutotileKey): RawImage {
  ...
  for (const [x, y] of GRIT_AT) {
    ...
    const isBase =
      img.data[i + 3] === 255 &&
      img.data[i] === ((ROAD_BASE >> 16) & 0xff) &&
      img.data[i + 1] === ((ROAD_BASE >> 8) & 0xff) &&
      img.data[i + 2] === (ROAD_BASE & 0xff)
...
export function paintRoadStrip(): RawImage {
  ...
  ROAD_AUTOTILE_KEYS.forEach((key, k) => {
    ...
    for (let y = 0; y < TILE_H; y++)
      strip.data.set(
        tile.data.subarray(y * TILE_W * 4, (y + 1) * TILE_W * 4),
        (y * width + k * TILE_W) * 4,
      )
  })
  return strip
