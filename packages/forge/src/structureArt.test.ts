...
import {
  DEFAULT_CONFIG, makeCityTemplate, CITY_DWELLING_KINDS, parseBuildingManifest,
} from '@sj/shared'
...
/** Where a kind's footprint is stated. The template and the recipe table are both authorities
 *  and they agree on `house`; the dev town's four are asserted in the gateway, next to it. */
const AUTHORITATIVE_FOOTPRINT = new Map<string, { w: number; h: number }>([
  ...TEMPLATE.structures.map((s) => [s.kind, { w: s.w, h: s.h }] as const),
...
describe('the committed cells', () => {
  ...
  it.each(cells.map((c) => [c.dir, c] as const))('%s clears the pixel bar', async (_dir, c) => {
    ...
    const fp = AUTHORITATIVE_FOOTPRINT.get(c.kind)
    // the four dev-town kinds are asserted against TOWN_STRUCTURES in the gateway's own test
    if (fp !== undefined) {
      expect(c.manifest.footprint, 'the manifest footprint is the world footprint').toEqual(fp)
    }
