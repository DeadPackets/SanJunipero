...
import {
  ONE_CELL_KINDS,
  SE_MIRROR_MIN_DISTANCE,
  STOREHOUSE_KIND,
  TWO_FACING_KINDS,
  facingPartitionIsTotal,
  mirrorFacingGate,
  requiredFacings,
...
describe('every kind the WORLD CAN CREATE has a cell, in every facing it can stand in', () => {
  it('is measuring the world it thinks it is: four sources, not one', () => {
    expect([...new Set(TEMPLATE.structures.map((s) => s.kind))].sort()).toEqual([
      'cabin',
      'cottage',
      'farmhouse',
      'fire_pit',
  ...
  it('NO KIND IS EXEMPT: every creatable kind either turns or says why it cannot', () => {
    ...
    expect(Object.keys(ONE_CELL_KINDS).sort()).toEqual([
      'bridge',
      'fire_pit',
      'grave',
      'lamp_post',
  ...
  it('MISSING: no kind the world can create is left to the procedural block', () => {
    ...
    expect(
      missing,
      `the world can create these and no codex cell answers:\n  ${missing.join('\n  ')}`,
    ).toEqual([])
  })
  ...
  it('ORPHAN: no cell is registered under a kind nothing can create', () => {
    ...
    expect(
      orphans,
      `registered, but nothing in the world is ever this kind:\n  ${orphans.join('\n  ')}`,
    ).toEqual([])
  })
  ...
  it('a kind the template stands but the recipe table has never heard of is still covered', () => {
    ...
    expect(requiredFacings(TEMPLATE.structures, CREATABLE).get('fire_pit')).toEqual(new Set(['sw']))
    expect(requiredFacings(TEMPLATE.structures, CREATABLE).get('shed')).toEqual(
      new Set(['sw', 'se']),
    )
...
describe('the committed cells', () => {
  ...
  it('ship one directory per kind and facing the world needs', () => {
    const want = [...requiredFacings(TEMPLATE.structures, CREATABLE)]
      .flatMap(([k, fs]) => [...fs].map((f) => facingKind(k, f)))
      .sort()
  ...
  it.each(cells.map((c) => [c.dir, c] as const))('%s clears the pixel bar', async (_dir, c) => {
    ...
    expect([img.width, img.height], 'authored at the size the 4x stop draws').toEqual([
      cellPx,
      cellPx,
    ])
    ...
    expect(
      nativeDensityGate({
        ...
        footprint: c.manifest.footprint,
        tile: TOWN_TILE,
      }).failures,
    ).toEqual([])
  ...
  it('turn the building instead of flipping it', async () => {
    ...
    expect(
      r.failures,
      r.measured.map((m) => `${m.kind} ${m.distance.toFixed(4)}`).join(', '),
    ).toEqual([])
