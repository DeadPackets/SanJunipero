...
describe('the migration', () => {
  it('runs twice without complaint and backfills every existing row as a tier-1 engine first', () => {
    ...
    db.prepare(
      'INSERT INTO milestones (kind, label, event_seq, day, tick) VALUES (?, ?, ?, ?, ?)',
...
describe('tier 1 — the engine firsts', () => {
  it('fires each predicate exactly once over a stream that triggers it three times', () => {
    const streams: Record<string, SimEvent[]> = {
      ...
      first_house: [
        ev(0, 'structure_planned', { id: 's1', kind: 'house' }),
        ev(1, 'structure_completed', { id: 's1' }),
      ],
      first_bridge: [
        ev(0, 'structure_planned', { id: 's2', kind: 'bridge' }),
        ev(1, 'structure_completed', { id: 's2' }),
      ],
      ...
      first_road: [
        ev(1, 'tile_changed', { x: 1, y: 1, from: 0, to: 7, reason: 'paved', byId: 'a' }),
      ],
      first_channel: [
        ev(1, 'tile_changed', { x: 1, y: 1, from: 0, to: 2, reason: 'channel', byId: 'a' }),
      ],
      first_fire: [ev(1, 'fire_ignited', { structureId: 's1', cause: 'hearth' })],
      first_fire_out: [
        ev(1, 'fire_extinguished', { structureId: 's1', cause: 'doused', agentId: 'a' }),
      ],
      first_inscription: [
        ev(1, 'structure_inscribed', { structureId: 's1', text: 'ours', agentId: 'a' }),
      ],
  ...
  it('counts the first sickness however the engine of the day recorded it', () => {
    ...
    const poison = detectFirsts(
      [ev(1, 'agent_afflicted', { agentId: 'a', kind: 'poison', severity: 1 })],
      ctx(),
    )
  ...
  it('gives every way of dying its own first, and each fires independently', () => {
    const stream = DEATH_CAUSES.map((cause, i) =>
      ev(i + 1, 'agent_died', { agentId: `a${i}`, cause }),
    )
  ...
  it('marks the turn of the year and the winter come through, once each', () => {
    ...
    expect(
      detectFirsts(
        [ev(60, 'agent_spoke', { agentId: 'a', text: 'day one', x: 0, y: 0 })],
        ctx(),
      ).map((m) => m.kind),
...
describe('tier 2 — the patterns, and the parting that needs more than a gap', () => {
  ...
  it('an already-seen kind is never a first twice', () => {
    ...
    expect(
      detectTier2(births, {
        seenKinds: new Set(['first_conversation']),
