...
export const TIER1_DEFS: MilestoneDef[] = [
  // The C7 rows, unchanged in kind and label so a town's existing ledger still reads true.
  {
    ...
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'agent_spoke',
    agentIds: one('agentId'),
  },
  {
    kind: 'first_structure',
    label: 'the first thing built',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'structure_completed',
  },
  {
    kind: 'first_fire',
    ...
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'fire_ignited',
  },
  {
    kind: 'first_injury',
    label: 'the first wound',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'agent_injured',
    agentIds: one('agentId'),
  },
  {
    kind: 'first_death',
  ...
  },
  {
    ...
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'action_completed' && verbOf(ev) === 'give',
    agentIds: one('agentId'),
  },
  {
    kind: 'first_harvest',
    label: 'the first harvest',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'crop_harvested',
  },
  ...
  {
    kind: 'first_infection',
    label: 'the first sickness',
    tier: 1,
    domain: 'engine',
    match: (ev) =>
      ev.type === 'agent_infected' || (ev.type === 'agent_afflicted' && p(ev).kind === 'illness'),
    agentIds: one('agentId'),
  },
  {
    kind: 'first_recovery',
    label: 'the first recovery',
    tier: 1,
    domain: 'engine',
  ...
  {
    kind: 'first_house',
    label: 'the first roof of their own',
    tier: 1,
    domain: 'engine',
    match: (ev, ctx) =>
      ev.type === 'structure_completed' && ctx.structureKind?.(String(p(ev).id)) === 'house',
  },
  {
    kind: 'first_bridge',
    ...
    domain: 'engine',
    match: (ev, ctx) =>
      ev.type === 'structure_completed' && ctx.structureKind?.(String(p(ev).id)) === 'bridge',
  },
  {
    kind: 'first_meal',
    label: 'the first meal eaten',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'action_completed' && verbOf(ev) === 'eat',
    agentIds: one('agentId'),
  },
  {
    kind: 'first_fish',
    label: 'the first fish taken',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'action_completed' && verbOf(ev) === 'fish',
    agentIds: one('agentId'),
  },
  {
    kind: 'first_hunt',
    label: 'the first beast brought down',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'fauna_killed' && typeof p(ev).byId === 'string',
    agentIds: one('byId'),
  },
  {
    kind: 'first_tool',
    label: 'the first tool made',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'item_spawned' && p(ev).durability !== undefined,
  },
  {
    kind: 'first_expert_craft',
    label: "the first work worth a maker's mark",
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'item_spawned' && typeof p(ev).crafterMark === 'string',
    agentIds: one('crafterMark'),
  },
  {
    kind: 'first_theft',
    label: 'the first taking that was seen',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'item_taken' && p(ev).takerId !== p(ev).ownerId,
    agentIds: one('takerId'),
  },
  {
    kind: 'first_road',
    label: 'the first stretch of road',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'tile_changed' && p(ev).reason === 'paved',
    agentIds: one('byId'),
  },
  {
    kind: 'first_channel',
    label: 'the first water led to the fields',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'tile_changed' && p(ev).reason === 'channel',
    agentIds: one('byId'),
  },
  {
    kind: 'first_fire_out',
    label: 'the first fire beaten back',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'fire_extinguished' && p(ev).cause === 'doused',
    agentIds: one('agentId'),
  },
  {
    kind: 'first_inscription',
    label: 'the first words cut into a wall',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'structure_inscribed',
    agentIds: one('agentId'),
  },
  {
    kind: 'first_invention',
    label: 'the first craft nobody had done before',
    tier: 1,
    domain: 'engine',
    match: (ev) =>
      ev.type === 'action_completed' &&
      typeof verbOf(ev) === 'string' &&
      String(verbOf(ev)).startsWith('recipe:'),
    agentIds: one('agentId'),
  },
  {
    kind: 'first_expression',
    label: 'the first act done for its own sake',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'agent_expressed',
    agentIds: one('agentId'),
  },
  {
    kind: 'first_pregnancy',
    label: 'the first child on the way',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'agent_conceived',
    agentIds: (ev) => [String(p(ev).motherId), String(p(ev).fatherId)],
  },
  {
    kind: 'first_birth',
    label: 'the first child born here',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'agent_born',
    agentIds: one('id'),
  },
  {
    kind: 'first_grave',
    ...
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'grave_placed',
    agentIds: one('agentId'),
  },
  {
    kind: 'first_world_grown',
    label: 'the world is wider than it was',
    tier: 1,
    domain: 'engine',
    match: (ev) => ev.type === 'world_grown',
  },
  {
    kind: 'first_winter_survived',
    label: 'the first winter come through',
    tier: 1,
    domain: 'engine',
    match: (ev) => isFirstDayOfSpringAfterAWinter(ev.tick),
  },
  {
    kind: 'first_year',
    label: 'a whole year in this valley',
    tier: 1,
    domain: 'engine',
