...
import { PUBLIC_EVENT_TYPES, collectPublicRecord, publicRecordText, writeBiography } from './publications.js'
...
const seedWorld = (): Database.Database => {
  ...
  ins.run(100, 'agent_spoke', JSON.stringify({ agentId: 'tamar', text: 'The river turns.', x: 1, y: 1 }))
...
describe('collectPublicRecord', () => {
  ...
  it('respects throughDay and unknown agents', () => {
    ...
    world
      ...
      .run(3000, 'agent_spoke', JSON.stringify({ agentId: 'tamar', text: 'Day two words.', x: 1, y: 1 }))
...
describe('publicRecordText', () => {
  it('renders diegetic lines per event type', () => {
    expect(
      publicRecordText({ seq: 1, tick: 100, type: 'agent_spoke', payload: { agentId: 'tamar', text: 'The river turns.' } }),
    ).toContain('was heard to say: "The river turns."')
    expect(
      publicRecordText({ seq: 2, tick: 200, type: 'agent_died', payload: { agentId: 'tamar', cause: 'fever' } }),
    ).toContain('died of fever')
    expect(
      publicRecordText({ seq: 3, tick: 300, type: 'action_completed', payload: { agentId: 'tamar', verb: 'eat' } }),
...
describe('writeBiography', () => {
  it('persists a biography from the public record with null citations', async () => {
    ...
    const row = await writeBiography({
      store, llm: bioLlm('She was heard to speak of the river turning.'), world,
      agentId: 'tamar', name: 'Tamar', throughDay: 0,
    })
  ...
  it('handles an agent with no public record: deterministic body, no LLM call', async () => {
    ...
    const row = await writeBiography({ store, llm, world, agentId: 'ghost', name: 'Ghost', throughDay: 0 })
