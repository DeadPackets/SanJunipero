...
function deckAnswerAt(world: WorldState, x: number, y: number): string | null {
  ...
  for (const [dx, dy] of [
    ...
    const a = buildSiteOf(at(world, x + dx, y + dy), SHOWCASE_CONFIG, 'wright', {
      kind: BRIDGE_KIND,
      x,
      y,
    })
...
describe('★ WHERE A DECK MAY STAND IN THE SHOWCASE, asked of the engine and not of me', () => {
  ...
  it('★ THE CHANNEL IS TWO TILES WIDE AT THE FORD AND THREE AT EVERY OTHER WET ROW', () => {
    ...
    expect(
      two.map((y) => y - two[0]!),
      'the ford is not four contiguous rows',
    ).toEqual([0, 1, 2, 3])
  })
  ...
  it('★ AND EXACTLY THE FOUR FORD ROWS ACCEPT A DECK — nowhere else in the channel does', () => {
    ...
    for (const a of answers)
      if (a.refusal !== null) tally.set(a.refusal, (tally.get(a.refusal) ?? 0) + 1)
    expect([...tally.keys()].sort()).toEqual([
      'a bridge belongs over water',
      'both ends must reach something solid',
    ])
  })
  ...
  it('★ and the site the dev world hands its bridgewright is one the engine accepts', () => {
    ...
    expect(
      accepted.map((a) => `${a.x},${a.y}`),
      'the demo names a crossing the water refuses',
...
describe('★ THE WRIGHT STOPS WHEN THE DECK IS UP, and the reason it needed its own guard', () => {
  ...
  it('★ asks for the deck while the water is open, and for nothing once it is decked', () => {
    ...
    expect(bridgewrightIntent(open, SHOWCASE_CONFIG, 'wright', deck)).toEqual({
      verb: 'build',
      params: { kind: BRIDGE_KIND, x: deck.x, y: deck.y },
    })
    const crossed = fold(
      open,
      {
    ...
    expect(
      bridgewrightIntent(rested(done), SHOWCASE_CONFIG, 'wright', deck),
      'a rested wright starts a second deck on the first one',
    ).toBeNull()
  })
})
...
describe('★ THE DEV WORLD CROSSES ITS OWN RIVER — a founder builds a deck, the far bank opens', () => {
  ...
  const west = Object.values(run.state.structures).filter(
    (s) => s.kind === 'house' && s.x + s.w <= mid - RIVER_HALF,
  )
  console.log(
  ...
  it('★ A FOUNDER LAYS A DECK, through the real build verb, in the world a viewer opens', () => {
    ...
    for (let dx = 0; dx < deck.w; dx++) {
      expect(run.state.terrain[deck.y]![deck.x + dx], 'a plank of the deck is not over water').toBe(
        WATER,
      )
    }
    expect(run.state.terrain[deck.y]![deck.x - 1], 'the west end reaches no bank').not.toBe(WATER)
    expect(run.state.terrain[deck.y]![deck.x + deck.w], 'the east end reaches no spit').not.toBe(
      WATER,
    )
  })
  ...
  it('★ AND IT IS THE DECK AND NOTHING CACHED: take it away and the claim goes back east', () => {
    ...
    expect(without!.site.x, 'the far bank is still offered with no deck standing').toBeGreaterThan(
      mid,
    )
    ...
    expect(
      claimInWorld(half, { along: 2, deep: 2 })!.site.x,
      'a half-built deck opened the bank',
    ).toBeGreaterThan(mid)
  })
    ...
    for (const s of west) {
      for (let dy = 0; dy < s.h; dy++)
        for (let dx = 0; dx < s.w; dx++) {
          expect(
            run.state.terrain[s.y + dy]![s.x + dx],
            `${s.x + dx},${s.y + dy} is water`,
          ).not.toBe(WATER)
        }
    }
  })
