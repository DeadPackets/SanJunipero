...
IDS.forEach((id, i) => {
  ...
  state = fold(state, store.append(state.tick, 'agent_spawned',
    { id, name: id, x: at.x, y: at.y, sex: 'f', ageDays: 30 * 364 }), config)
  state = fold(state, store.append(state.tick, 'item_spawned',
    { id: `wood_${id}`, kind: 'wood', qty: 10, loc: { t: 'agent', id }, owner: id }), config)
})

const lawQueue: LawQueue = []
const worldTick = createWorldTick(config, rngOf(), lawQueue)
function rngOf(): RngStreams { return new RngStreams('motive-sanity') }
let handler: TickHandler = () => {}
const loop = new TickLoop({ store, state, rng: rngOf(), config, startTick: START_TICK, realMsPerTick: 0, onTick: (c) => handler(c) })
...
console.log(`other structures: ${Object.values(loop.state.structures).map((s) => s.kind).sort().join(', ')}`)
const spot = groundForBuilding(loop.state)
console.log(`ground the town keeps for a roof: ${spot === null ? 'NONE — the probe cannot work' : `(${spot.x}, ${spot.y})`}`)
console.log(`founders at: ${IDS.map((id) => `${id}(${loop.state.agents[id]!.x},${loop.state.agents[id]!.y})`).join(' ')}`)
if (spot !== null) {
  const d = IDS.map((id) => Math.abs(loop.state.agents[id]!.x - spot.x) + Math.abs(loop.state.agents[id]!.y - spot.y))
...
if (spot !== null) {
  ...
  for (let i = 0; i < 400 && (loop.state.agents['yusuf']!.x !== spot.x || loop.state.agents['yusuf']!.y !== spot.y); i++) loop.step()
  const y = loop.state.agents['yusuf']!
  console.log(`yusuf walked to (${y.x}, ${y.y}) in ${loop.tick - START_TICK - 1} ticks`)
  let verdict = 'no answer'
  void bridge.submit('yusuf', { verb: 'build', params: { kind: 'house' } }, (r) => { verdict = JSON.stringify(r) })
  loop.step()
  console.log(`build house: ${verdict}`)
  for (let i = 0; i < 600; i++) loop.step()
  console.log(`structures now: ${Object.values(loop.state.structures).map((s) => `${s.kind}:${s.stage}`).sort().join(', ')}`)
}
