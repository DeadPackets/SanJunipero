...
const PASSING: G11Report = {
  ...
  preflight: {
    ...
    passed: true,
    costUsd: 0.0009,
    speechAdvisory: 'speech 2/3 — ADVISORY, not gated',
    roundsRun: 1,
    roundsPassed: 1,
    servedProviders: ['Baidu'],
    failures: [],
  },
  opsPlane: {
  ...
  measurements: {
    ...
    faunaCount: 26,
    forageableCount: 20,
    snapshotBytes: 214_880,
    foldMsFromGenesis: 91.4,
    foldEvents: 51_233,
    medianTickMs: 0.6,
    p99TickMs: 4.1,
  },
  spend: {
    ...
    outputTokens: 82_000,
    cacheReadTokens: 122_400,
    cacheReadShare: 0.068,
    costPerMindPerSimDay: 0.231,
    requestedProviderOrder: ['Baidu'],
  ...
  evidence: {
    ...
    reflectionsStarted: 4,
    reflectionsResolved: 4,
    deadCalls: { calls: 12, emptyOutput: 8, unparseable: 3, otherFailures: 1 },
    deadCallAlertRows: 2,
    constructs: {
      expressiveVerbs: ['express:sing'],
      firstExpressiveBy: 'amara',
      reusedBy: 'yusuf',
      reuseArbiterCalls: 0,
      passRan: true,
      passErrors: 0,
    ...
    semanticHits: [],
    semanticPassErrors: 0,
    semanticUnreadableNights: 1,
...
const failing = (r: G11Report): string[] =>
  Object.entries(checkG11Report(r))
    .filter(([, d]) => d !== null)
    .map(([k]) => k)
...
describe('the gate criteria', () => {
  ...
  it('a town that never drank, never ate what it gathered, or never saw the dark, fails', () => {
    expect(
      failing(
        broken((r) => {
          r.evidence.unpromptedDrinks = 0
        }),
      ),
    ).toContain('2.thirst-reaches-an-act')
    expect(
      failing(
        broken((r) => {
          r.evidence.gatheredFoodEaten = 0
        }),
      ),
    ).toContain('3.food-gathered-then-eaten')
    expect(
      failing(
        broken((r) => {
          r.evidence.darkPerceptions = 0
        }),
      ),
    ).toContain('10.a-night-somebody-stood-in')
  })
  ...
  it('silence around the sick founder fails, and both a recovery and a death pass', () => {
    expect(
      failing(
        broken((r) => {
          r.evidence.stagedAfflictionResponses = []
        }),
      ),
    ...
    expect(
      failing(
        broken((r) => {
          r.evidence.stagedAfflictionResolved = 'died'
        }),
      ),
    ...
    expect(
      failing(
        broken((r) => {
          r.evidence.stagedAfflictionAgentId = null
        }),
      ),
    ).toContain('4.the-sick-founder-is-answered')
  })
  ...
  it('one banned word anywhere in the chronicle fails the whole scan', () => {
    expect(
      failing(
        broken((r) => {
          r.evidence.chronicleViolations = ['number: she lost 12']
        }),
      ),
  ...
  it('the laws panel has to list all fourteen C11 flags, flip one, and still replay', () => {
    expect(
      failing(
        broken((r) => {
          r.evidence.worldLawPaths = r.evidence.worldLawPaths.slice(0, 13)
        }),
      ),
    ...
    expect(
      failing(
        broken((r) => {
          r.evidence.replayHashMatches = false
        }),
      ),
    ).toContain('7.laws-listed-flipped-and-replayed')
  })
  ...
  it('a lost reflection fails, and dead calls nobody was told about fail', () => {
    expect(
      failing(
        broken((r) => {
          r.evidence.reflectionsResolved = 3
        }),
      ),
  ...
  it('the second body has to be a DIFFERENT body, and it has to cost nothing', () => {
    expect(
      failing(
        broken((r) => {
          r.evidence.constructs.reusedBy = 'amara'
        }),
      ),
    ...
    expect(
      failing(
        broken((r) => {
          r.evidence.constructs.reuseArbiterCalls = 1
        }),
      ),
    ).toContain('9.constructs-live')
    expect(
      failing(
        broken((r) => {
          r.evidence.constructs.passRan = false
        }),
      ),
    ).toContain('9.constructs-live')
    expect(
    ...
    expect(
      failing(
        broken((r) => {
          r.evidence.constructs.recognized = 0
        }),
      ),
  ...
  it('the measurement rows must be readings and not zeroes', () => {
    expect(
      failing(
        broken((r) => {
          r.measurements.trafficKeys = 0
        }),
      ),
    ).toContain('B.measured-on-a-grown-map')
    expect(
      failing(
        broken((r) => {
          r.measurements.snapshotBytes = 0
        }),
      ),
    ).toContain('B.measured-on-a-grown-map')
  })
  ...
  it('the three named world assertions each fail on their own', () => {
    expect(
      failing(
        broken((r) => {
          r.evidence.fordBridge.buildable = false
        }),
      ),
    ...
    expect(
      failing(
        broken((r) => {
          r.evidence.farBankWalk.stoppedAtWaterEdge = false
        }),
      ),
    ...
    expect(
      failing(
        broken((r) => {
          r.evidence.clothedSurviveLadder = false
        }),
      ),
