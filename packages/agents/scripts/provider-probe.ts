...
import { runPreflight, type PreflightAnswer, type PreflightResult } from '../src/live/providerPreflight.js'
...
async function publishedPrices(): Promise<Map<string, { prompt: number; completion: number }>> {
  ...
  try {
    ...
    const body = await res.json() as { data?: { endpoints?: Array<{ provider_name?: string; pricing?: { prompt?: string; completion?: string } }> } }
...
async function main(): Promise<void> {
  ...
  const num = (sql: string, ...p: unknown[]): number => Number(db.prepare(sql).pluck().get(...p))
  const rows: Row[] = []
  ...
  for (const c of CANDIDATES.filter((x) => only.length === 0 || only.includes(x.name))) {
    const llm = new LlmClient({
      db, caller: c.name, providerOrder: c.order, allowProviderFallbacks: !c.hardAllowList,
      ...
      maxRetries: 0, requestTimeoutMs: 90_000,
    })
    const answers: PreflightAnswer[] = []
    const rounds: PreflightResult[] = []
    for (let round = 0; round < ROUNDS; round++) {
      const result = await runPreflight({
        llm, provider: c.name, hardAllowList: c.hardAllowList, model: MIND_MODEL,
        onAnswer: (a) => answers.push(a),
        costUsd: () => num('SELECT COALESCE(SUM(cost_usd),0) FROM llm_calls WHERE caller = ?', c.name),
        servedProviders: () => (db.prepare(
          'SELECT DISTINCT provider FROM llm_calls WHERE caller = ? AND provider IS NOT NULL')
          .all(c.name) as Array<{ provider: string }>).map((r) => r.provider),
      })
      rounds.push(result)
      console.log(`[probe] ${c.name} round ${round + 1}/${ROUNDS}: action ${result.actions}/${result.calls},`
      ...
      if (spent > CAP_USD) { console.error(`STOP: $${spent.toFixed(4)} past the $${CAP_USD} probe cap`); process.exit(1) }
    }
    const sum = (pick: (r: PreflightResult) => number): number => rounds.reduce((a, r) => a + pick(r), 0)
    const served = [...new Set(rounds.flatMap((r) => r.servedProviders))].sort()
    rows.push({
      candidate: c.name, hardAllowList: c.hardAllowList, rounds,
      roundsPassed: rounds.filter((r) => r.passed).length,
      actions: sum((r) => r.actions), speeches: sum((r) => r.speeches),
      answered: sum((r) => r.answered), calls: sum((r) => r.calls),
      costUsd: num('SELECT COALESCE(SUM(cost_usd),0) FROM llm_calls WHERE caller = ?', c.name),
      servedProviders: served, failures: rounds.flatMap((r) => r.failures),
      inputTokens: num('SELECT COALESCE(SUM(input_tokens),0) FROM llm_calls WHERE caller = ?', c.name),
      outputTokens: num('SELECT COALESCE(SUM(output_tokens),0) FROM llm_calls WHERE caller = ?', c.name),
      cacheReadTokens: num('SELECT COALESCE(SUM(cache_read_tokens),0) FROM llm_calls WHERE caller = ?', c.name),
  ...
  writeFileSync(OUT_PATH, JSON.stringify({ generatedAt: new Date().toISOString(), model: MIND_MODEL, totalCostUsd: total, rows }, null, 2))
  ...
  for (const r of rows) {
    ...
    console.log(`| ${r.candidate} | ${r.hardAllowList} | ${r.servedProviders.join(',') || 'unattributed'}`
  ...
  console.log(passers.length === 0
    ? 'NO PINNED CANDIDATE PASSES EVERY ROUND. The gate runs unpinned with fallbacks on.'
    : `Pinned passers, cheapest first: ${[...passers].sort((a, b) => a.costUsd - b.costUsd).map((r) => r.candidate).join(', ')}`)
}
