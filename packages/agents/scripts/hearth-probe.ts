...
import {
  createWorldTick, EventStore, fold, genesisState, isExposed, makeGenesisWorld, RngStreams,
  ...
  ? SimConfigSchema.parse({
      structures: {
        recipes: {
          ...
          house: { ...SimConfigSchema.parse({}).structures.recipes['house']!, hearth: false, bed: false },
        },
      },
    })
  : SimConfigSchema.parse({})
...
async function main(): Promise<void> {
  ...
  const fireOf = (id: string): string | null => {
    ...
    return at === undefined ? null : ((s.structures[at]?.fueledUntilTick ?? 0) > s.tick ? 'lit' : 'cold')
  }
  ...
  const report = {
    ...
    hearthsStanding: Object.values(s.structures)
      ...
      .map((x) => `${x.kind} ${x.id}`).sort(),
    shelterLedger: (() => { const l = shelterLedger(s, config); return { ...l, per: Number(l.per.toFixed(2)) } })(),
    ...
    firesLitAtDawn: Object.values(s.structures)
      .filter((x) => (x.fueledUntilTick ?? 0) > s.tick).map((x) => `${x.kind} ${x.id}`).sort(),
    ...
    energyAtEnd: Object.fromEntries(MINDS.map((m) =>
      [m.id, Number((s.agents[m.id]?.needs.energy ?? -1).toFixed(1))])),
    warmthAtEnd: Object.fromEntries(MINDS.map((m) =>
      [m.id, Number((s.agents[m.id]?.needs.warmth ?? -1).toFixed(1))])),
    warmthTargetAtEnd: Object.fromEntries(MINDS.map((m) =>
      [m.id, Number(warmthTargetFor(s, config, m.id).toFixed(1))])),
    ...
    thoughtsMentioningFire: thoughts.filter((t) => /\bhearth|fire|flame|ember|kindl|warm the|smoke\b/i.test(t.text)).length,
    thoughtsMentioningBed: thoughts.filter((t) => /\bbed|sleep|lie down|rest\b/i.test(t.text)).length,
  ...
  writeFileSync(path.join(DATA_DIR, `${LABEL}-speech.md`),
    events.filter((e) => e.type === 'agent_spoke').map((e) => `- t=${e.tick} ${JSON.stringify(e.payload)}`).join('\n'))
