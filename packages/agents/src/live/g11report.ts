...
export const G11EvidenceSchema = z
  .object({
    ...
    socialSurvivalActs: z
      .object({
        tends: z.number().int(),
        gives: z.number().int(),
        jointBuilds: z.number().int(),
        sharedFireMeals: z.number().int(),
      })
      .strict(),
  })
  .strict()
...
export const G11ReportSchema = z
  .object({
...
export function checkG11Report(report: G11Report): Record<string, string | null> {
  ...
  return {
    '1.two-days-no-crash-budget-held': (() => {
      ...
      const ok =
        e.ticksRun === report.totalTicks &&
        e.crashAlerts === 0 &&
        e.drainedAgainCount === 0 &&
        days >= G11_MIN_SIM_DAYS &&
        e.overBudgetTicks === 0
      return ok
        ? null
    ...
    '2.thirst-reaches-an-act':
      e.unpromptedDrinks >= 1 ? null : `unprompted drinks=${e.unpromptedDrinks}`,
    '3.food-gathered-then-eaten':
      e.foodGathered >= 1 && e.gatheredFoodEaten >= 1
        ? null
        : `gathered=${e.foodGathered} eaten=${e.gatheredFoodEaten}`,
    // Recovery or death both pass; silence fails.
    '4.the-sick-founder-is-answered':
      e.stagedAfflictionAgentId === null
    ...
    '5.chronicle-says-nothing-it-should-not':
      e.chronicleLines >= 1 && e.chronicleViolations.length === 0
        ? null
        : `lines=${e.chronicleLines} violations=${e.chronicleViolations.slice(0, 3).join(' | ')}`,
    '6.feet-wear-the-routes-feet-take':
      e.tilesWorn >= 1
        ? null
        : `maxTraffic=${e.maxTileTraffic} threshold=${e.wearThreshold} worn=${e.tilesWorn}`,
    '7.laws-listed-flipped-and-replayed': (() => {
      const ok =
        e.worldLawPaths.length >= 14 &&
        e.lawFlips.length >= 1 &&
        e.lawHistoryEntries >= 1 &&
        e.replayHashMatches
      return ok
        ? null
    ...
    '9.constructs-live': (() => {
      const cheap =
        c.expressiveVerbs.length >= 1 &&
        c.reusedBy !== null &&
        c.reusedBy !== c.firstExpressiveBy &&
        c.reuseArbiterCalls === 0
    ...
    '10.a-night-somebody-stood-in':
      e.darkPerceptions >= 1 ? null : 'no perception packet read dark',
    '11.the-nightly-semantic-pass-runs-clean':
    ...
    'C.the-ford-takes-a-bridge': e.fordBridge.buildable
      ? null
      : `the ford at (${e.fordBridge.x}, ${e.fordBridge.y}) refuses: ${e.fordBridge.refusal}`,
    'D.the-far-bank-stops-at-the-water':
      e.farBankWalk.refused && e.farBankWalk.stoppedAtWaterEdge
        ? null
        : `refused=${e.farBankWalk.refused} reason=${e.farBankWalk.reason} edge=${e.farBankWalk.stoppedAtWaterEdge}`,
