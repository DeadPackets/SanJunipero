...
describe('MemoryStore round-trip', () => {
  ...
  it('memoriesOfDay returns only that day, in insert order', async () => {
    const { store } = await makeStore()
    const a = await store.insertMemory({
      tick: 10,
      kind: 'thought',
      text: 'day zero',
      importance: 2,
    ...
    const b = await store.insertMemory({
      tick: 20,
      kind: 'thought',
      text: 'day zero too',
      importance: 2,
...
describe('sqlite-vec KNN', () => {
  it('same text embedding returns the row first with distance ~ 0', async () => {
    ...
    await store.insertMemory({
      tick: 8,
      kind: 'action',
      text: 'a funeral in deep winter snow',
      importance: 5,
      tags: TAGS,
    })
    await store.insertMemory({
...
describe('ledgers', () => {
  it('upsert overwrites the doc and bumps updatedDay', async () => {
    ...
    store.upsertLedger(
      'omar',
      'Omar helped, then argued about the grain split. [mem#12][mem#19]',
      5,
    )
...
describe('facts, summaries, journal, autobiography, miss log', () => {
  ...
  it('autobiography appends and reads back paragraphs in order', async () => {
    ...
    expect(store.autobiography()).toEqual([
