...
describe('/api/bonds — the deterministic proxy that stands in for C9 T11/T12', () => {
  ...
  beforeAll(async () => {
    ...
    const loop = new TickLoop({
      ...
      onTick: ({ tick, emit }) => {
        ...
        if (tick === 5)
          emit('agent_spoke', { agentId: 'cara', text: 'Fine morning.', x: 20, y: 20 })
  ...
  const find = (a: string, b: string): Bond | undefined =>
    body.bonds.find((x) => x.id === bondId(a, b))
  ...
  it('ties a child to each parent', () => {
    for (const parent of ['alice', 'bob']) {
      ...
      expect(
        b?.recent.map((h) => bondNote(h.kind)),
        parent,
      ).toEqual(['parent and child'])
  ...
  it('reads a gift as a debt, a lesson as work, and a blow as a rivalry', () => {
    ...
    expect(find('dan', 'eve')?.recent.map((h) => bondNote(h.kind))).toEqual([
      'taught something',
      'came to blows',
    ])
  })
...
describe('★ the talk window is a window, not the whole log', () => {
  ...
  it('ties exactly the pairs inside the window and none outside it', () => {
    const events = [
      spoke(1, 0, 'a', 0),
      spoke(2, 5, 'b', 1), // 5 apart, in earshot → friend
      spoke(3, 200, 'c', 0),
      spoke(4, 219, 'a', 1), // 19 apart → friend
      spoke(5, 240, 'b', 0),
      spoke(6, 261, 'c', 1), // 21 apart → NOT a pair
      spoke(7, 400, 'a', 0),
      spoke(8, 401, 'b', 40), // in the window, out of earshot
    ]
    ...
    expect(out.bonds.find((b) => b.id === bondId('a', 'c'))?.recent.map((h) => h.tick)).toEqual([
      219,
    ])
  })
