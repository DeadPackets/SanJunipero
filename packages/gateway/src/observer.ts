...
export function latestThought(
  db: Database.Database,
  agentId: string,
): { tick: number; text: string } | null {
  const r = db
    .prepare('SELECT tick, text FROM observer_thoughts WHERE agent_id = ? ORDER BY id DESC LIMIT 1')
    .get(agentId) as { tick: number; text: string } | undefined
  return r ?? null
}
