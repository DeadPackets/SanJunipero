...
const collect = ((): (() => void) => {
  ...
  if (exposed) return () => { for (let i = 0; i < 5; i++) exposed() }
  setFlagsFromString('--expose-gc')
  const gc = runInNewContext('gc') as () => void
  setFlagsFromString('--no-expose-gc')
  return () => { for (let i = 0; i < 5; i++) gc() }
})()
const heapMB = (): number => { collect(); return process.memoryUsage().heapUsed / 1024 / 1024 }
  ...
  it('three times the events must not be three times the memory', () => {
    ...
    const loop = new TickLoop({
      ...
      onTick: ({ tick, emit }) => {
        ...
        for (let k = 0; k < 6; k++) {
          emit('agent_moved', { id: ids[Math.floor(rnd() * AGENTS)]!, x: Math.floor(rnd() * 60), y: Math.floor(rnd() * 60) })
        }
        if (tick % 3 === 0) {
          emit('action_started', {
        ...
        if (tick % 37 === 0) emit('agent_injured', { agentId: ids[Math.floor(rnd() * AGENTS)]!, kind: 'minor' })
      },
    })
    ...
    let footprint = (): Footprint => { throw new Error('api.ts never offered a footprint') }
    mountDataApi({ route: (m, p, fn) => { routes.set(`${m} ${p}`, fn) } },
      { db: apiDb, mirror, config: DEFAULT_CONFIG, onFootprint: (f) => { footprint = f } })

    const bodies = new Map<string, string>()
    const warm = (): void => {
      for (const [key, url] of [
        ['GET /api/society', '/api/society'], ['GET /api/heat', '/api/heat'],
        ['GET /api/digest', '/api/digest'],
      ] as Array<[string, string]>) {
        let body = ''
        routes.get(key)!({ url } as IncomingMessage,
    ...
    expect(f.heat, 'one entry per 60-tick window per agent in it')
      .toBeLessThanOrEqual(Math.ceil(mirror.state().tick / 60 + 1) * AGENTS)
