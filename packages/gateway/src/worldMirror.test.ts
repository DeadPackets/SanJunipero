...
import { EventStore, RngStreams, TickLoop, genesisState, openDb, replayFromGenesis, fold, type TileId } from '@sj/engine'
...
describe('WorldMirror', () => {
  ...
  it('boots to the live state, polls new ticks, and scrubs with stateAt', () => {
    ...
    const mirror = new WorldMirror({ db: new Database(dbPath, { readonly: true }), config: DEFAULT_CONFIG, terrain: GRASS })
    ...
    expect(stateHash(mirror.state())).toBe(stateHash(replayFromGenesis(store, DEFAULT_CONFIG, GRASS)))
    ...
    const ref6 = upTo6.reduce((s, ev) => fold(s, ev, DEFAULT_CONFIG), genesisState(DEFAULT_CONFIG, GRASS))
