...
const VerdictProbe = z.discriminatedUnion('kind', [
  ...
  z.object({ kind: z.literal('attempt'), recipeId: z.string().describe('a short stable slug for this recipe, lowercase with underscores, e.g. "extract_salt_by_boiling"'), summary: z.string() }).strict()
...
console.log(`[probe] model=${MIND_MODEL} budget=$${CAP_USD.toFixed(2)} caller=probe systemChars=${SYSTEM.length}`)

// ---- Check 1: structured verdict ----
{
  const r = await llm.object({
    system: SYSTEM,
    messages: [{ role: 'user', content: 'Agent intent: "I try to extract salt by boiling river water." Return a verdict.' }],
    schema: VerdictProbe,
  })
  budget()
  const v = r.value
  if (v.kind !== 'attempt') fail(`check1 kind=${v.kind}, expected attempt: ${JSON.stringify(v)}`)
  if (!v.summary || v.summary.trim().length === 0) fail(`check1 summary empty: ${JSON.stringify(v)}`)
  console.log(`CHECK 1 PASS: kind=${v.kind} summary=${JSON.stringify(v.summary)}`)
  console.log(`  in=${r.usage.inputTokens} out=${r.usage.outputTokens} cacheRead=${r.usage.cacheReadTokens} cost=$${r.usage.costUsd.toFixed(6)}`)
}
...
// ---- Check 2: canon prefix cache ----
{
  ...
  console.log(`CHECK 2 PASS: cacheReadTokens=${cin} (in=${r2.usage.inputTokens} out=${r2.usage.outputTokens} cost=$${r2.usage.costUsd.toFixed(6)})`)
}
...
// ---- Check 3: rephrase consistency (evidence) ----
{
  const a = await llm.object({
    system: SYSTEM,
    messages: [{ role: 'user', content: 'Agent intent: "I boil river water to get salt." Return a verdict.' }],
    schema: VerdictProbe,
  })
  const b = await llm.object({
    system: SYSTEM,
    messages: [{ role: 'user', content: 'Agent intent: "I extract salt from the river by boiling." Return a verdict.' }],
    schema: VerdictProbe,
  })
  budget()
  if (a.value.kind !== 'attempt') fail(`check3a kind=${a.value.kind}, expected attempt: ${JSON.stringify(a.value)}`)
  if (b.value.kind !== 'attempt') fail(`check3b kind=${b.value.kind}, expected attempt: ${JSON.stringify(b.value)}`)
  console.log('CHECK 3 PASS (evidence): rephrase consistency')
  console.log(`  recipeId(a)=${JSON.stringify(a.value.recipeId)}  recipeId(b)=${JSON.stringify(b.value.recipeId)}`)
