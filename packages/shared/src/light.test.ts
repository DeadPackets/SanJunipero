...
const torchAt = (x: number, y: number, litUntilTick: number): LitWorld =>
  world({
    items: { item_1: { kind: 'torch', litUntilTick, loc: { t: 'tile', x, y } } },
  })

const pit = (x: number, y: number, w: number, h: number, fueledUntilTick?: number): LitWorld =>
  world({
    structures: {
      structure_1: {
        kind: 'fire_pit',
...
describe('the glow table is the config, read in one place', () => {
  it('names a radius for each source the world knows, and nothing else', () => {
    expect(LIGHT_GLOW_RADIUS).toEqual({
      torch: 3,
      lantern: 5,
      hearth: 3,
      fire_pit: 4,
...
describe('lightLevelAt: the day is free, the night is not', () => {
  ...
  it('a torch whose fuel ran out is a stick', () => {
    expect(lightLevelAt(torchAt(10, 10, MIDNIGHT - 1), 10, 10, MIDNIGHT, CFG)).toBe(
      CFG.nightWitness.nightFactor,
    )
    expect(
      lightLevelAt(
        world({ items: { item_1: { kind: 'torch', loc: { t: 'tile', x: 10, y: 10 } } } }),
        10,
        10,
        MIDNIGHT,
        CFG,
  ...
  it('a carried torch lights the tile the carrier stands on', () => {
    const carried = world({
      agents: { a1: { x: 20, y: 20 } },
      items: {
        item_1: { kind: 'torch', litUntilTick: MIDNIGHT + 10, loc: { t: 'agent', id: 'a1' } },
      },
    })
  ...
  it('an unfed pit and one still under construction throw no light at all', () => {
    expect(lightLevelAt(pit(10, 10, 1, 1), 10, 10, MIDNIGHT, CFG)).toBe(
      CFG.nightWitness.nightFactor,
    )
    const raising = world({
      structures: {
...
describe('visionRadiusAt: the light that matters is the light on the thing seen', () => {
  ...
  it('is full sight by day and scales by the darkness factor at night', () => {
    ...
    expect(visionRadiusAt(world(), eye, 30, 30, MIDNIGHT, CFG)).toBe(
      Math.round(SIGHT * CFG.nightWitness.nightFactor),
    )
    expect(visionRadiusAt(world(), eye, 30, 30, DUSK, CFG)).toBe(
      Math.round(SIGHT * CFG.nightWitness.duskFactor),
    )
  })
  ...
  it('a torch at the target restores full sight; the same torch at the eye does not', () => {
    ...
    expect(visionRadiusAt(atEye, eye, 30, 30, MIDNIGHT, CFG)).toBe(
      Math.round(SIGHT * CFG.nightWitness.nightFactor),
    )
  })
  ...
  it('is exactly the band saying "dark" — one derivation, not a second threshold', () => {
    ...
    for (const tick of [NOON, DUSK, MIDNIGHT]) {
      for (const [x, y] of [
        ...
        expect([tick, x, y, isDark(lit, x, y, tick, CFG)]).toEqual([
          tick,
          x,
          y,
          lightBandAt(lit, x, y, tick, CFG) === 'dark',
        ])
      }
    }
  })
