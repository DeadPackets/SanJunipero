...
export const centreOf = (s: {
  ...
  w: number
  h: number
}): { sx: number; sy: number } => screenOf(s.dx + s.w / 2, s.dy + s.h / 2)
...
export const isRiverWater = (dx: number, dy: number): boolean => Math.abs(dx - riverCentre(dy)) < 3
...
export function blockTiles(i: number, j: number): TileXY[] {
  ...
  for (let dy = r.dy0; dy <= r.dy1; dy++)
    for (let dx = r.dx0; dx <= r.dx1; dx++) out.push({ dx, dy })
  return out
}
...
export function freePlots(rings: number, ground: Ground): Plot[] {
  ...
  return out.sort(
    (a, b) =>
      Math.max(Math.abs(a.block.i), Math.abs(a.block.j)) -
        Math.max(Math.abs(b.block.i), Math.abs(b.block.j)) ||
...
export function closestPair(structures: readonly Massed[]): number {
  ...
  for (let a = 0; a < structures.length; a++)
    for (let b = a + 1; b < structures.length; b++) {
      const p = centreOf(structures[a]!),
        q = centreOf(structures[b]!)
...
export function townSpacing(
  governed: readonly Massed[],
  ungoverned: readonly Massed[],
): TownSpacing {
  return {
...
export function townErrors(
  ...
  for (let a = 0; a < structures.length; a++)
    for (let b = a + 1; b < structures.length; b++) {
      ...
      if (d < MIN_SEP)
        errs.push(`${structures[a]!.kind} and ${structures[b]!.kind} are ${d.toFixed(1)} px apart`)
    }
  for (const s of structures) {
    ...
    for (let j = i + 1; j < ps.length; j++)
      for (const [a1, d1] of sizes) {
        ...
        for (const [a2, d2] of sizes) {
          ...
          pairings++
          if (placedTiles(s2).some((t) => t1.has(`${t.dx},${t.dy}`))) {
            overlaps++
            continue
          }
          const p = centreOf(s1),
          ...
          if (d < closest) {
            closest = d
            worst =
              `(${s1.block.i},${s1.block.j}) ${s1.slot} ${a1}×${d1}  ↔  ` +
...
export function townExtent(
  structures: readonly { dx: number; dy: number; w: number; h: number }[],
): {
  ...
  let dx0 = Infinity,
    ...
    dy1 = -Infinity
  for (const s of structures) {
    dx0 = Math.min(dx0, s.dx)
    dy0 = Math.min(dy0, s.dy)
    dx1 = Math.max(dx1, s.dx + s.w - 1)
    dy1 = Math.max(dy1, s.dy + s.h - 1)
  }
  return {
    tiles: { dx0, dy0, dx1, dy1 },
    // The screen box of a tile box is its four CORNERS: west is (dx0,dy1), east is (dx1,dy0).
    screen: {
      sx0: screenOf(dx0, dy1).sx,
      sy0: screenOf(dx0, dy0).sy,
      sx1: screenOf(dx1, dy0).sx,
      sy1: screenOf(dx1, dy1).sy,
    },
  }
}
