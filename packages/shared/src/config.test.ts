...
describe('SimConfigSchema: C9 living-world sections', () => {
  ...
  it('a roof is a property of the kind, not a roster of names', () => {
    ...
    const roofed = Object.keys(c.structures.recipes)
      .filter((k) => isRoofedKind(c, k))
      .sort()
  ...
  it('a fire is a property of the kind, not a roster of names', () => {
    const hearths = Object.keys(c.structures.recipes)
      .filter((k) => isHearthKind(c, k))
      .sort()
    ...
    for (const k of ['well', 'bridge', 'grave', 'storehouse'])
      expect(isHearthKind(c, k), k).toBe(false)
  ...
  it('the furnishings the engine acts on are exactly the ones the room is drawn with', () => {
    const furnishedWith = (kind: string): string[] =>
      [
        ...new Set(
          cityStructures()
    ...
    const dwellingsWith = (has: (c: typeof DEFAULT_CONFIG, k: string) => boolean): string[] =>
      Object.keys(c.structures.recipes)
        .filter((k) => has(c, k) && isRoofedKind(c, k))
        .sort()
  ...
  it("the row carries the two furnishings a law reads and not the room's whole inventory", () => {
    ...
    expect(
      Object.keys(row)
        .filter(
          (k) => !['inputs', 'w', 'h', 'maxHp', 'flammable', 'durationTicks', 'sited'].includes(k),
        )
        .sort(),
    ).toEqual(['bed', 'hearth', 'roofed'])
    // and every one of those three IS in the room the template draws
    const inTheRoom = cityStructures()
      .find((s) => s.kind === 'house')!
      .furnishings.map((f) => f.kind)
  ...
  it('spoilage: per-kind days, preserving structures', () => {
    expect(c.spoilage.days).toEqual({
      fish: 2,
      berries: 3,
      rabbit_meat: 3,
      venison: 4,
  ...
  it('mortality, illness and thirst carry their exact dials', () => {
    ...
    expect(c.mortality.thirstHpDrainPerTick).toBe(0.08)
  ...
  it('warmth and light carry the ratified values', () => {
    ...
    expect(c.light.glowRadius).toEqual({
      torch: 3,
      lantern: 5,
      hearth: 3,
      fire_pit: 4,
  ...
  it('constructs keeps an open taxonomy', () => {
    ...
    expect(c.constructs.types).toEqual({
      festival: true,
      faith: true,
      council: true,
      market: true,
