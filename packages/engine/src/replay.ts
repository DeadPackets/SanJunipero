...
export function replayLatest(
  ...
  terrain?: TileId[][],
  seed?: string,
): { state: WorldState; rng: RngStreams; seq: number } {
  ...
  if (ckpt) rng = RngStreams.restore(ckpt.rng)
    ...
    throw new Error(
      'replayLatest: no seed source (no rng checkpoint, no snapshot, and no seed given)',
    )
  ...
  if (ckpt && events.length > 0 && ckpt.tick !== state.tick) {
    throw new Error(
      `rng checkpoint (tick ${ckpt.tick}) is behind folded state (tick ${state.tick})`,
    )
  }
  if (!ckpt && snap && events.length > 0 && snap.tick !== state.tick) {
    throw new Error(
      `rng checkpoint (tick ${snap.tick}) is behind folded state (tick ${state.tick})`,
    )
  }
