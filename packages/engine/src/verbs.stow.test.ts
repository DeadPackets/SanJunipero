...
function world(stage: 'construction' | 'complete' = 'complete', kind = 'storehouse'): WorldState {
  let s = genesisState(DEFAULT_CONFIG, OPEN.map((row) => [...row].map((): TileId => 0)))
...
function withHolder(s: WorldState, x: number, y: number, owner = 'a1'): WorldState {
  ...
  return fold(spawned, ev('item_spawned', {
    id: 'item_1', kind: 'bread', qty: 1, loc: { t: 'agent', id: 'a1' }, owner,
  }))
}

const goInside = (s: WorldState, structureId = 'structure_1'): WorldState =>
  fold(fold(s, ev('agent_moved', { id: 'a1', x: 2, y: 3 })), ev('agent_entered', { agentId: 'a1', structureId }))

const MOVED = { type: 'item_moved', payload: { id: 'item_1', loc: { t: 'structure', id: 'structure_1' } } }

describe('verb: stow', () => {
  it('shelves a held thing from just outside the wall', () => {
    const s = withHolder(world(), 4, 2)
    expect(submitIntent(s, DEFAULT_CONFIG, 'a1', 'stow', { itemId: 'item_1', structureId: 'structure_1' }).ok).toBe(true)
    expect(VERBS.stow!.onComplete(s, DEFAULT_CONFIG, 'a1', { itemId: 'item_1', structureId: 'structure_1' }, RNG))
      .toEqual([MOVED])
  })
  ...
  it('leaves ownership exactly where it was — a shelf is not a transfer', () => {
    ...
    const after = VERBS.stow!.onComplete(s, DEFAULT_CONFIG, 'a1', { itemId: 'item_1', structureId: 'structure_1' }, RNG)
      .reduce((acc, e) => fold(acc, ev(e.type, e.payload)), s)
    expect(after.items.item_1!).toMatchObject({ owner: 'a2', loc: { t: 'structure', id: 'structure_1' } })
  })

  it('refuses a building that is still going up', () => {
    const s = withHolder(world('construction'), 4, 2)
    expect(submitIntent(s, DEFAULT_CONFIG, 'a1', 'stow', { itemId: 'item_1', structureId: 'structure_1' }))
      .toMatchObject({ ok: false, reason: 'it is not finished' })
  })

  it('refuses out of reach, and refuses what you are not holding', () => {
    expect(submitIntent(withHolder(world(), 6, 5), DEFAULT_CONFIG, 'a1', 'stow', { itemId: 'item_1', structureId: 'structure_1' }))
    ...
    expect(submitIntent(withHolder(world(), 4, 2), DEFAULT_CONFIG, 'a1', 'stow', { itemId: 'ghost', structureId: 'structure_1' }))
      .toMatchObject({ ok: false, reason: 'not holding that' })
    expect(submitIntent(withHolder(world(), 4, 2), DEFAULT_CONFIG, 'a1', 'stow', { itemId: 'item_1', structureId: 'ghost' }))
      .toMatchObject({ ok: false, reason: 'there is nothing there to put it in' })
    expect(submitIntent(withHolder(world(), 4, 2), DEFAULT_CONFIG, 'a1', 'stow', { itemId: 'item_1' }))
  ...
  it('a wall stops hands — indoors you can only shelve the room you are in', () => {
    ...
    s = fold(s, ev('structure_planned', {
      id: 'structure_2', kind: 'storehouse', x: 5, y: 1, w: 1, h: 1, maxHp: 20, flammable: true, builderId: 'a1',
    }))
    s = fold(s, ev('structure_completed', { id: 'structure_2' }))
    s = goInside(s)
    expect(submitIntent(s, DEFAULT_CONFIG, 'a1', 'stow', { itemId: 'item_1', structureId: 'structure_2' }))
