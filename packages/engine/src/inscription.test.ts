...
const apply = (s: WorldState, params: Record<string, unknown>): WorldState =>
  VERBS.inscribe!.onComplete(s, DEFAULT_CONFIG, 'a1', params, RNG).reduce(
    (acc, e) => fold(acc, ev(e.type, e.payload)),
    s,
  )
...
describe('verb: inscribe', () => {
  ...
  it('carves a house wall from just outside it', () => {
    ...
    expect(apply(s, params).structures.structure_1!.inscription).toEqual({
      text: 'here we kept the winter out',
      by: 'a1',
    })
  })
  ...
  it('carves the standing stone — if they mark it, that is their culture', () => {
    ...
    expect(submitIntent(s, DEFAULT_CONFIG, 'a1', 'inscribe', params).ok).toBe(true)
    expect(apply(s, params).structures.structure_1!.inscription!.text).toBe(
  ...
  it('overwriting keeps only the latest in state; the log keeps the palimpsest', () => {
    const s = withCarver(world(), 4, 2)
    const first = ev('structure_inscribed', {
      structureId: 'structure_1',
      text: 'first',
      agentId: 'a1',
    })
    const second = ev('structure_inscribed', {
      structureId: 'structure_1',
      text: 'second',
      agentId: 'a2',
    })
    ...
    expect([first, second].map((e) => (e.payload as { text: string }).text)).toEqual([
      'first',
      'second',
    ])
  })
  ...
  it('refuses an unfinished building, an absent one, and a wall out of reach', () => {
    ...
    expect(
      submitIntent(
        withCarver(world('house', 'construction'), 4, 2),
        DEFAULT_CONFIG,
        'a1',
        'inscribe',
        { structureId: 'structure_1', text },
      ),
    ).toMatchObject({ ok: false, reason: 'it is not finished' })
    expect(
      submitIntent(withCarver(world(), 4, 2), DEFAULT_CONFIG, 'a1', 'inscribe', {
        structureId: 'ghost',
        text,
      }),
    ).toMatchObject({ ok: false, reason: 'there is nothing there to mark' })
    expect(
      submitIntent(withCarver(world(), 9, 9), DEFAULT_CONFIG, 'a1', 'inscribe', {
        structureId: 'structure_1',
        text,
      }),
  ...
  it('a wall stops the chisel — indoors you can only mark the room you are in', () => {
    ...
    s = goInside(s)
    expect(
      submitIntent(s, DEFAULT_CONFIG, 'a1', 'inscribe', { structureId: 'structure_2', text: 'no' }),
    ).toMatchObject({ ok: false, reason: 'a wall is in the way' })
  })

  it('the fold throws for a structure that is not there', () => {
    expect(() =>
      fold(world(), ev('structure_inscribed', { structureId: 'ghost', text: 'x', agentId: 'a1' })),
...
describe('perception: inscriptions', () => {
  const inscribed = (): WorldState =>
    fold(
      world(),
      ev('structure_inscribed', { structureId: 'structure_1', text: 'the ford', agentId: 'a1' }),
    )
  ...
  it('from inside you read them too', () => {
    ...
    expect(
      composePerception(s, DEFAULT_CONFIG, 'a1', []).visible.structures[0]!.inscription,
