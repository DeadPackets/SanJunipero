...
function makeWorld(config = CFG, wood = 10): WorldState {
  ...
  if (wood > 0)
    s = fold(
      s,
      ev('item_spawned', { id: 'item_1', kind: 'wood', qty: wood, loc: { t: 'agent', id: 'a1' } }),
      config,
    )
  return { ...s, tick: NOON }
}
...
describe('verb: build', () => {
  it('validates kind, reach, placement, and materials', () => {
    ...
    expect(submitIntent(makeWorld(), CFG, 'a1', 'build', { kind: 'house', x: 1, y: 1 }).ok).toBe(
      true,
    )
  })
  ...
  it('consumes materials and plans the structure at action start', () => {
    ...
    expect(r.events).toContainEqual({
      ...
      payload: {
        agentId: 'a1',
        verb: 'build',
        params: { kind: 'house', x: 1, y: 1 },
        duration: CFG.construction.houseTicks,
      },
    })
    expect(r.events).toContainEqual({
    ...
    expect(w.structures.structure_2).toMatchObject({
      stage: 'construction',
      hp: 1,
      progressTicks: 0,
    })
  ...
  it('progresses the structure one tick per worked tick', () => {
    ...
    expect(r.events).toContainEqual({
      ...
      payload: { agentId: 'a1', ticks: 1 },
    })
    expect(r.events).toContainEqual({
      type: 'structure_progressed',
      payload: { id: 'structure_2', ticks: 1 },
    })
  ...
  it('interrupt preserves progress; resume needs no new materials and completes after houseTicks total', () => {
    ...
    expect(done.events).toContainEqual({
      ...
      payload: { id: 'structure_2' },
    })
    expect(done.events).toContainEqual({
      type: 'skill_gained',
      payload: { agentId: 'a1', track: 'carpentry', xp: 1 },
    })
    expect(done.state.structures.structure_2).toMatchObject({
      stage: 'complete',
      hp: 50,
      progressTicks: 3,
    })
...
describe('verb: build reads structures.recipes', () => {
  ...
  it('a well is 1x1, costs its row in stone, and does not burn', () => {
    ...
    expect(r.events).toContainEqual({
      type: 'item_qty_changed',
      payload: { id: 'item_1', delta: -WELL.inputs.stone! },
    })
    expect(r.events).toContainEqual({
      type: 'action_started',
      payload: {
        agentId: 'a1',
        verb: 'build',
        params: { kind: 'well', x: 1, y: 1 },
        duration: WELL.durationTicks,
      },
    })
  ...
  it('one stone short is one stone short, by the name of the material', () => {
    const r = submitIntent(withStone(WELL.inputs.stone! - 1), CFG, 'a1', 'build', {
      kind: 'well',
      x: 1,
      y: 1,
    })
...
describe('verb: fill', () => {
  ...
  it('fills a waterskin to its config charge count and a bucket to exactly one', () => {
    ...
    expect(skin.events).toContainEqual({
      type: 'item_filled',
      payload: { itemId: 'item_1', charges: CFG.thirst.waterskinCharges },
    })
