...
function world(...agents: Array<{ id: string; x: number; y: number }>): WorldState {
  let s = genesisState(DEFAULT_CONFIG, ROWS.map((row) => [...row].map((c) => CHAR_TILE[c]!)))
  for (const a of agents) s = fold(s, ev('agent_spawned', { id: a.id, name: a.id, x: a.x, y: a.y, ageDays: 7300 }))
  return s
}

function apply(s: WorldState, events: Array<{ type: string; payload: unknown }>, config = DEFAULT_CONFIG): WorldState {
...
function withLooseItem(s: WorldState, owner?: string): WorldState {
  return fold(s, ev('item_spawned', {
    id: 'item_1', kind: 'bread', qty: 1, loc: { t: 'tile', x: 1, y: 1 },
...
describe('acquisition marks the acquirer', () => {
  ...
  it('the flag turns acquisition ownership off entirely', () => {
    ...
    const payload = events.find(e => e.type === 'item_spawned')!.payload as Record<string, unknown>
...
describe('taking', () => {
  ...
  it('moves another agent’s thing without moving its ownership, and says so out loud', () => {
    ...
    expect(events).toEqual([
      { type: 'item_moved', payload: { id: 'item_1', loc: { t: 'agent', id: 'a1' } } },
      { type: 'item_taken', payload: { itemId: 'item_1', kind: 'bread', takerId: 'a1', ownerId: 'a2', x: 1, y: 1 } },
    ])
  ...
  it('picking up your own thing again is not a theft and not a claim', () => {
    ...
    expect(VERBS.take!.onComplete(s, DEFAULT_CONFIG, 'a1', { itemId: 'item_1' }, RNG))
      .toEqual([{ type: 'item_moved', payload: { id: 'item_1', loc: { t: 'agent', id: 'a1' } } }])
  })
  ...
  it('with the flag off, taking is a bare move', () => {
    const s = withLooseItem(world({ id: 'a1', x: 1, y: 1 }, { id: 'a2', x: 4, y: 4 }), 'a2')
    expect(VERBS.take!.onComplete(s, OFF, 'a1', { itemId: 'item_1' }, RNG))
      .toEqual([{ type: 'item_moved', payload: { id: 'item_1', loc: { t: 'agent', id: 'a1' } } }])
  })
  ...
  it('reports the shelf position when the thing was taken from a structure', () => {
    ...
    s = fold(s, ev('structure_planned', {
      id: 'structure_1', kind: 'storehouse', x: 2, y: 2, w: 2, h: 1, maxHp: 20, flammable: true, builderId: 'a2',
    }))
    s = fold(s, ev('item_spawned', {
      id: 'item_1', kind: 'bread', qty: 1, loc: { t: 'structure', id: 'structure_1' }, owner: 'a2',
    }))
    expect(VERBS.take!.onComplete(s, DEFAULT_CONFIG, 'a1', { itemId: 'item_1' }, RNG).at(-1))
      .toEqual({ type: 'item_taken', payload: { itemId: 'item_1', kind: 'bread', takerId: 'a1', ownerId: 'a2', x: 2, y: 2 } })
  })
})

describe('giving', () => {
  it('hands over the thing and the title with it', () => {
    const s = fold(world({ id: 'a1', x: 1, y: 1 }, { id: 'a2', x: 2, y: 1 }),
      ev('item_spawned', { id: 'item_1', kind: 'bread', qty: 1, loc: { t: 'agent', id: 'a1' }, owner: 'a1' }))
    const events = VERBS.give!.onComplete(s, DEFAULT_CONFIG, 'a1', { itemId: 'item_1', targetId: 'a2' }, RNG)
    expect(events).toEqual([
  ...
  it('with the flag off, giving is a bare move', () => {
    const s = fold(world({ id: 'a1', x: 1, y: 1 }, { id: 'a2', x: 2, y: 1 }),
      ev('item_spawned', { id: 'item_1', kind: 'bread', qty: 1, loc: { t: 'agent', id: 'a1' }, owner: 'a1' }))
    expect(VERBS.give!.onComplete(s, OFF, 'a1', { itemId: 'item_1', targetId: 'a2' }, RNG))
      .toEqual([{ type: 'item_moved', payload: { id: 'item_1', loc: { t: 'agent', id: 'a2' } } }])
  })
})

describe('ownership outlives the owner and the transfer', () => {
  it('a dead owner’s dropped thing is still theirs', () => {
    let s = fold(world({ id: 'a1', x: 3, y: 4 }),
      ev('item_spawned', { id: 'item_1', kind: 'bread', qty: 1, loc: { t: 'agent', id: 'a1' }, owner: 'a1' }))
  ...
  it('a maker’s mark survives every hand the thing passes through', () => {
    let s = fold(world({ id: 'a1', x: 1, y: 1 }, { id: 'a2', x: 2, y: 1 }),
      ev('item_spawned', {
        id: 'item_1', kind: 'chair', qty: 1, loc: { t: 'agent', id: 'a1' }, owner: 'a1', crafterMark: 'a1',
      }))
    s = apply(s, VERBS.give!.onComplete(s, DEFAULT_CONFIG, 'a1', { itemId: 'item_1', targetId: 'a2' }, RNG))
...
describe('item_taken is a witness record, not a mechanic', () => {
  it('folds to a state byte-identical to the one before it', () => {
    const s = withLooseItem(world({ id: 'a1', x: 1, y: 1 }, { id: 'a2', x: 4, y: 4 }), 'a2')
    const after = fold(s, ev('item_taken', {
      itemId: 'item_1', kind: 'bread', takerId: 'a1', ownerId: 'a2', x: 1, y: 1,
    }))
  ...
  it('item_owner_changed rejects an item that is not there', () => {
    ...
    expect(() => fold(s, ev('item_owner_changed', { id: 'ghost', owner: 'a1' }))).toThrow(/unknown item/)
  })
})
