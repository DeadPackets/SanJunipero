...
function withStorehouse(s: WorldState, config = DEFAULT_CONFIG): WorldState {
  const planned = fold(s, ev('structure_planned', {
    id: 'structure_1', kind: 'storehouse', x: 4, y: 4, w: 2, h: 1, maxHp: 20, flammable: true, builderId: 'a1',
  }), config)
  return fold(planned, ev('structure_completed', { id: 'structure_1' }), config)
}

type Loc = { t: 'tile'; x: number; y: number } | { t: 'agent'; id: string } | { t: 'structure'; id: string }

function withFood(s: WorldState, kind: string, loc: Loc, spawnDay = 0, config = DEFAULT_CONFIG): WorldState {
  const days = config.spoilage.days[kind]
  return fold(s, ev('item_spawned', {
    id: 'item_1', kind, qty: 1, loc,
...
function midnightEvents(s: WorldState, day: number, config = DEFAULT_CONFIG): Array<{ type: string; payload: unknown }> {
  const tick = day * MINUTES_PER_DAY
  const advanced = fold({ ...s, tick: tick - 1 }, ev('tick_advanced', {}, tick), config)
  return createWorldTick(config, new RngStreams('spoilage'))(advanced).events.filter(e => e.type === 'item_spoiled')
}
...
describe('the midnight spoilage check', () => {
  it('takes the fish at the midnight that ends its second day', () => {
    ...
    expect(midnightEvents(liveThrough(s, 1), 2)).toEqual([{ type: 'item_spoiled', payload: { id: 'item_1' } }])
  ...
  it('reads the multiplier from where the loaf sits now, not from where it was baked', () => {
    ...
    const shelved = fold(liveThrough(held, 5), ev('item_moved', { id: 'item_1', loc: { t: 'structure', id: 'structure_1' } }))
    ...
    const pocketed = fold(liveThrough(kept, 6), ev('item_moved', { id: 'item_1', loc: { t: 'agent', id: 'a1' } }))
  ...
  it('only fires at midnight', () => {
    const s = withFood(world(), 'fish', { t: 'agent', id: 'a1' })
    const noon = fold({ ...s, tick: 2 * MINUTES_PER_DAY + 719 }, ev('tick_advanced', {}, 2 * MINUTES_PER_DAY + 720))
    expect(createWorldTick(DEFAULT_CONFIG, new RngStreams('spoilage'))(noon).events.filter(e => e.type === 'item_spoiled'))
      .toEqual([])
  })
...
describe('what leaves the hand with a shelf life', () => {
  it('stamps only the kinds the table names', () => {
    ...
    expect(spoilageFor({ ...s, tick: 3 * MINUTES_PER_DAY }, 'bread', DEFAULT_CONFIG))
      .toEqual({ spoilage: { spawnDay: 3, days: 6 } })
  })

  it('carries the shelf life out of the woods on foraged berries', () => {
    const s = { ...world(), tick: MINUTES_PER_DAY }
    const spawned = VERBS.forage!.onComplete(s, DEFAULT_CONFIG, 'a1', {}, RNG).find(e => e.type === 'item_spawned')!
  ...
  it('leaves a written note out of it — paper is not food', () => {
    const spawned = VERBS.write!.onComplete(world(), DEFAULT_CONFIG, 'a1', { text: 'hello' }, RNG)[0]!
...
describe('perception smells it coming', () => {
  const perceive = (s: WorldState, config = DEFAULT_CONFIG) => composePerception(s, config, 'a1', [])
  ...
  it('says nothing about a loaf that the storehouse is still holding back', () => {
    ...
    const near = { ...s, agents: { a1: { ...s.agents.a1!, x: 4, y: 3 } }, tick: 5 * MINUTES_PER_DAY }
  ...
  it('goes quiet with the flag off', () => {
    const s = withFood(world(), 'fish', { t: 'tile', x: 1, y: 1 })
    expect(perceive({ ...s, tick: MINUTES_PER_DAY }, OFF).visible.items[0]).not.toHaveProperty('spoiling')
  })
})
