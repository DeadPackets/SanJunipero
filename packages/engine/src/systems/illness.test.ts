...
const cfg = (illness: Record<string, unknown>): SimConfig =>
  SimConfigSchema.parse({ ...quiet, illness })
...
const of = (r: WorldTickResult, type: string) =>
  r.events.filter((e) => e.type === type).map((e) => e.payload)
const illnessOf = (s: WorldState, id: string) =>
  s.agents[id]!.afflictions?.find((x) => x.kind === 'illness')
...
describe('illnessSystem: the fever turns at midnight', () => {
  ...
  it("steps down when the roll goes the body's way, and lifts at the last step", () => {
    ...
    expect(of(three, 'affliction_worsened')).toEqual([
      { agentId: 'a1', kind: 'illness', severity: 2 },
    ])
  ...
  it('turns only at midnight, and only for a body that has the fever', () => {
    ...
    const noon = createWorldTick(
      WORSENS,
      new RngStreams('ill'),
    )(fold({ ...s, tick: 719 }, ev('tick_advanced', {}, 720), WORSENS))
...
describe('illnessSystem: contagion, once a night, on the illness stream', () => {
  it('crosses a shared roof to every co-occupant and stops at the walls', () => {
    ...
    for (const id of ['a1', 'a2', 'a3'])
      s = fold(s, ev('agent_entered', { agentId: id, structureId: 'structure_1' }), SPREADS)
...
describe('illnessSystem: a wound turns septic at dawn', () => {
  ...
  const wound = (config: SimConfig, tick = 0): WorldState =>
    fold(
      town(config, [['a1', 2, 2]]),
      ev('agent_injured', { agentId: 'a1', kind: 'minor' }, tick),
      config,
    )
