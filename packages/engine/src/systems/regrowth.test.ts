...
function tickOnce(s: WorldState, config = CFG): WorldTickResult {
  return createWorldTick(config, new RngStreams('rg'))(fold(s, ev('tick_advanced', {}, s.tick + 1), config))
}
const seeded = (r: WorldTickResult) => r.events
  .filter((e) => e.type === 'tile_changed' && (e.payload as { reason: string }).reason === 'seeded')
...
describe('seeding: the forest edge creeps back into the grass', () => {
  it('seeds every orthogonal neighbour on a certain roll, and no diagonal one', () => {
    ...
    expect(out.map((p) => [p.x, p.y]).sort()).toEqual([[1, 2], [2, 1], [2, 3], [3, 2]])
  ...
  it('stamps the day it was seeded, sparsely, and nowhere else', () => {
    ...
    expect(s.saplings).toEqual({
      [saplingKey(1, 2)]: 1, [saplingKey(2, 1)]: 1, [saplingKey(2, 3)]: 1, [saplingKey(3, 2)]: 1,
    })
  ...
  it('never seeds a tile that is not grass', () => {
    ...
    const terrain = paved.terrain.map((row, y) => row.map((t, x) => (x === 1 && y === 2 ? (7 as TileId) : t)))
    expect(seeded(tickOnce({ ...paved, terrain }, SURE)).map((p) => [p.x, p.y]))
...
describe('maturity: a sapling is a forest thirty days later, to the day', () => {
  ...
  const NOSEED: SimConfig = SimConfigSchema.parse({ ...quiet, regrowth: { saplingChancePerDay: 0 } })
  const planted = (): WorldState => tickOnce(wood(SURE, midnightOf(1)), SURE).state
  const grown = (r: WorldTickResult) => r.events
    .filter((e) => e.type === 'tile_changed' && (e.payload as { reason: string }).reason === 'grown')
  ...
  it('does not grow on the day before, and grows on the day itself', () => {
    ...
    expect(grown(due).map((p) => [p.x, p.y]).sort()).toEqual([[1, 2], [2, 1], [2, 3], [3, 2]])
  ...
  it('drops the stamp when it grows, so a grown wood hashes like one that was always there', () => {
    ...
    const t = Array.from({ length: 6 }, () => Array.from({ length: 6 }, (): TileId => 0))
    for (const [x, y] of [[2, 2], [1, 2], [2, 1], [2, 3], [3, 2]]) t[y!]![x!] = 3
    // Same clock and same sky, so the only thing left to differ is whether the ground still
    // remembers being planted. It does not.
    const always: WorldState = { ...genesisState(NOSEED, t), tick: due.state.tick, weather: due.state.weather }
...
describe('a sapling is ground you can walk on and ground you can clear', () => {
  const sapling = (config = CFG): WorldState => {
    ...
    s = fold(s, ev('agent_spawned', { id: 'a1', name: 'a1', x: 2, y: 0, ageDays: 7300 }, 720), config)
    return s
  }
  ...
  it('is chopped back to grass, and yields nothing at all', () => {
    ...
    const out = VERBS.chop!.onComplete(s, CFG, 'a1', { x: 2, y: 1 }, new RngStreams('c').get('actions'))
    expect(out).toEqual([{
      type: 'tile_changed', payload: { x: 2, y: 1, from: 9, to: 0, reason: 'cleared', byId: 'a1' },
    }])
...
describe('forageable nodes climb back toward what the world authored', () => {
  ...
  const bush = (stock: number, fullStock?: number): WorldState => {
    const s = { ...genesisState(REGROW, Array.from({ length: 4 }, () => Array.from({ length: 4 }, (): TileId => 0))), tick: DAWN - 1 }
    return fold(s, ev('forageable_spawned', {
      id: 'node_1', kind: 'berry_bush', x: 1, y: 1, stock,
  ...
  it('regrows a picked bush one handful at a time, and stops at what it was', () => {
    ...
    const full = createWorldTick(REGROW, new RngStreams('fg2'))(fold(s, ev('tick_advanced', {}, s.tick + 1), REGROW))
