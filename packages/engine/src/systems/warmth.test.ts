...
function bodyAt(tick: number, config = CFG, extra: Partial<AgentBody> = {}): WorldState {
  ...
  s = fold(s, ev('agent_spawned', { id: 'a1', name: 'a1', x: 4, y: 4, ageDays: 7300 }, tick - 1), config)
...
function tickOnce(s: WorldState, config = CFG): WorldTickResult {
  return createWorldTick(config, new RngStreams('wa'))(fold(s, ev('tick_advanced', {}, s.tick + 1), config))
}

type NeedEv = { id: string; need: string; delta: number; reason?: string }
const needs = (r: WorldTickResult, need: string): NeedEv[] =>
  r.events.filter((e) => e.type === 'need_changed' && (e.payload as NeedEv).need === need)
    .map((e) => e.payload as NeedEv)
const chills = (r: WorldTickResult): NeedEv[] => r.events
  .filter((e) => e.type === 'need_changed' && (e.payload as NeedEv).reason === 'exposure')
  .map((e) => e.payload as NeedEv)

const skyOf = (tick: number, kind: string): WorldState =>
  ({ ...genesisState(CFG, map()), tick, weather: { kind, temperatureC: 0 } })

describe('ambientTempAt: a deterministic table, never a roll', () => {
  it('returns the ratified band for every season and phase', () => {
    const table: Array<[number, number]> = [
      [at(10, 12), 14], [at(10, 6), 9], [at(10, 22), 5],
      [at(100, 12), 26], [at(100, 6), 20], [at(100, 22), 15],
      [at(200, 12), 10], [at(200, 19), 6], [at(200, 22), 2],
      [at(273, 12), -4], [at(273, 5), -8], [at(273, 22), -12],
    ]
...
describe('exposure: what the cold takes, and who it cannot reach', () => {
  ...
  it('a warm enough day takes nothing: the body drifts to the weather, as it always did', () => {
    const r = tickOnce(bodyAt(SUMMER_DAY, CFG, { needs: { hunger: 100, energy: 100, warmth: 50, social: 100 } }))
  ...
  it('four walls, a kept fire and a garment each stop it', () => {
    ...
    const beside = (fueledUntilTick: number): WorldState => {
      let s = bodyAt(WINTER_NIGHT)
      s = fold(s, ev('structure_planned', {
        id: 'structure_1', kind: 'fire_pit', x: 6, y: 4, w: 1, h: 1, maxHp: 10, flammable: false, builderId: 'a1',
      }, s.tick), CFG)
      s = fold(s, ev('structure_completed', { id: 'structure_1' }, s.tick), CFG)
      return { ...s, structures: { ...s.structures, structure_1: { ...s.structures.structure_1!, fueledUntilTick } } }
    }
  ...
  it('a garment offsets the band by its insulation, and that is what decides an autumn dusk', () => {
    ...
    let dressed = fold(bare, ev('item_spawned', {
      id: 'item_1', kind: 'garment', qty: 1, loc: { t: 'agent', id: 'a1' },
    }, bare.tick), CFG)
    dressed = { ...dressed, agents: { ...dressed.agents, a1: { ...dressed.agents.a1!, equipped: { body: 'item_1' } } } }
  ...
  it('leaves the dead and the unborn alone', () => {
    const dead = fold(bodyAt(WINTER_NIGHT), ev('agent_died', { agentId: 'a1', cause: 'hunger' }), CFG)
...
describe('a body with no warmth left burns twice the energy', () => {
  ...
  it('the doubling is what puts the body on the ground on the tick it falls', () => {
    ...
    const warm = tickOnce(bodyAt(SUMMER_DAY, CFG, { needs: { hunger: 100, energy: 5.1, warmth: 100, social: 100 } }))
  ...
  it('a sleeper is chilled but not drained: sleep is the recovery, not the cost', () => {
    const asleep = bodyAt(WINTER_NIGHT, CFG, { asleep: true, needs: { hunger: 100, energy: 40, warmth: 0, social: 100 } })
...
describe('the cold names the death it drove, and nothing else', () => {
  const failing = (tick: number, warmth: number) => {
    const s = bodyAt(tick, CFG, { hp: 0.1, needs: { hunger: 100, energy: 60, warmth, social: 100 } })
    return fold(s, ev('agent_afflicted', { agentId: 'a1', kind: 'fatigue', severity: 3 }, s.tick), CFG)
  }
...
describe('determinism', () => {
  it('folding the tick\'s own events reproduces the state it returned', () => {
    const start = bodyAt(WINTER_NIGHT, CFG, { needs: { hunger: 100, energy: 80, warmth: 0, social: 100 } })
