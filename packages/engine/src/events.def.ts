...
// Applied in the order the laws queued them, so a batch folds to exactly the state the
// separate events left.
...
export const ItemFilled = z
  .object({ itemId: z.string(), charges: z.number().int().nonnegative() })
  .strict()
// Pure witness record — folds to nothing. Whether a taking is theft is the town's to decide.
export const ItemTaken = z
  .object({
    itemId: z.string(),
    kind: z.string(),
    takerId: z.string(),
...
export const StructureFueled = z
  .object({
    structureId: z.string(),
    burnsUntilTick: z.number().int(),
  })
  .strict()
...
export const StructurePlanned = z
  .object({
    id: z.string(),
    kind: z.string(),
    x: z.number(),
...
export const StructureInscribed = z
  .object({
    structureId: z.string(),
...
// The added fields are optional so every recorded rain and burnout still parses.
export const FireExtinguished = z
  .object({
    structureId: z.string().optional(),
    cause: z.enum(['doused', 'rain', 'burnout']),
...
export const ActionCompleted = z
  .object({
    agentId: z.string(),
    verb: z.string(),
...
// `agentId` stays the PATIENT so recorded logs fold unchanged.
export const AgentTended = z
  .object({
    agentId: z.string(),
...
export const AgentHarmed = z
  .object({
    agentId: z.string(),
    amount: z.number().nonnegative(),
    source: z.enum(['attack', 'fire', 'accident']),
    byId: z.string().optional(),
  })
  .strict()
export const AgentAfflicted = z
  .object({
    agentId: z.string(),
    kind: AfflictionKindSchema,
    severity: z.number().positive(),
    sourceId: z.string().optional(),
    itemId: z.string().optional(),
  })
  .strict()
export const AfflictionWorsened = z
  .object({
    agentId: z.string(),
    kind: AfflictionKindSchema,
    severity: z.number().positive(),
  ...
  .strict()
export const AfflictionRecovered = z
  .object({ agentId: z.string(), kind: AfflictionKindSchema })
  .strict()
...
export const AgentDrank = z
  .object({
    agentId: z.string(),
    source: z.enum(['water_tile', 'well', 'item']),
    itemId: z.string().optional(),
  })
  .strict()
export const WeatherChanged = z
  .object({ kind: z.string(), temperatureC: z.number(), prevKind: z.string().optional() })
  .strict()
// Pure sensation: no fold effect, no cause, no resolution anywhere in the world.
export const MysteryEvent = z
  .object({
    kind: z.string(),
    x: z.number().int().optional(),
    y: z.number().int().optional(),
...
export const WildlifeChanged = z
  .object({ fish: z.number().optional(), deer: z.number().optional() })
  .strict()

// `stock` rides the spawn: a school arrives as a number of fish, not as one.
const FaunaKindSchema = z.enum(['deer', 'rabbit', 'fish'])
export const FaunaSpawned = z
  .object({
    id: z.string(),
    kind: FaunaKindSchema,
    x: z.number().int(),
    y: z.number().int(),
...
export const FaunaMoved = z
  .object({
    moves: z
      .array(z.object({ id: z.string(), x: z.number().int(), y: z.number().int() }).strict())
      .min(1),
  })
  .strict()
...
export const FaunaStockChanged = z
  .object({ id: z.string(), stock: z.number().int().positive() })
  .strict()
export const FaunaKilled = z
  .object({
    id: z.string(),
    kind: FaunaKindSchema,
    x: z.number().int(),
  ...
  .strict()
// A node is never removed: the last handful is a depletion, not a deletion.
...
// Optional so every recorded scatter still parses; absent means a ceiling of one.
export const ForageableSpawned = z
  .object({
    id: z.string(),
    kind: ForageableKindSchema,
    x: z.number().int(),
    y: z.number().int(),
  ...
  .strict()
export const ForageableStockChanged = z
  .object({ id: z.string(), stock: z.number().int().positive() })
  .strict()
export const ForageableDepleted = z.object({ id: z.string() }).strict()
export const ForageableRegrown = z
  .object({ id: z.string(), stock: z.number().int().positive() })
  .strict()
