...
const spawnItem = (id: string, qty = 5) =>
  ev(1, 'item_spawned', { id, kind: 'wood', qty, loc: { t: 'tile', x: 1, y: 1 } })
...
describe('fold items', () => {
  ...
  it('throws on item events for unknown items', () => {
    ...
    expect(() =>
      fold(s, ev(1, 'item_moved', { id: 'nope', loc: { t: 'tile', x: 0, y: 0 } })),
    ).toThrow(/unknown item/i)
    expect(() => fold(s, ev(1, 'item_qty_changed', { id: 'nope', delta: -1 }))).toThrow(
      /unknown item/i,
    )
  ...
  it('wears a tool down and breaks it, leaving the durability-free alone', () => {
    ...
    expect(() => fold(plain, ev(2, 'item_worn', { id: 'item_2', delta: -1 }))).toThrow(
      /no durability/i,
    )
  })
  it('strict payloads reject extra keys on item events', () => {
    ...
    expect(() =>
      fold(s, ev(2, 'item_moved', { id: 'item_1', loc: { t: 'tile', x: 0, y: 0, extra: 1 } })),
    ).toThrow()
    expect(() =>
      fold(s, ev(2, 'item_qty_changed', { id: 'item_1', delta: -1, extra: 1 })),
    ).toThrow()
  })
})
...
describe('fold structures', () => {
  ...
  it('throws on structure events for unknown structures', () => {
    const s = genesisState(DEFAULT_CONFIG)
    expect(() => fold(s, ev(1, 'structure_progressed', { id: 'nope', ticks: 1 }))).toThrow(
      /unknown structure/i,
    )
    expect(() => fold(s, ev(1, 'structure_completed', { id: 'nope' }))).toThrow(
      /unknown structure/i,
    )
    expect(() => fold(s, ev(1, 'structure_damaged', { id: 'nope', amount: 1 }))).toThrow(
      /unknown structure/i,
    )
    expect(() => fold(s, ev(1, 'structure_destroyed', { id: 'nope' }))).toThrow(
      /unknown structure/i,
    )
  })
  it('strict payloads reject extra keys on structure events', () => {
    ...
    expect(() =>
      fold(s, ev(2, 'structure_progressed', { id: 'structure_1', ticks: 1, extra: 1 })),
    ).toThrow()
    expect(() => fold(s, ev(2, 'structure_completed', { id: 'structure_1', extra: 1 }))).toThrow()
    expect(() =>
      fold(s, ev(2, 'structure_damaged', { id: 'structure_1', amount: 1, extra: 1 })),
    ).toThrow()
