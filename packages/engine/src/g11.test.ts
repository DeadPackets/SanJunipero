...
function pass(s: WorldState, config: SimConfig, tick: number, seed = 'g11a', laws?: LawQueue): {
...
function genesisTown(config: SimConfig = CFG): { state: WorldState; terrain: TileId[][]; events: PendingEvent[] } {
  const { terrain, events } = makeGenesisWorld(config)
  const state = events.reduce((s, e) => fold(s, ev(e.type, e.payload, 0), config), genesisState(config, terrain))
...
describe('G11a-M1: the genesis town folds, and it is the size the world says it is', () => {
  ...
  it('every tile a paving touches names its own chunk, and no other', () => {
    const { state } = genesisTown()
    const touched = [{ x: 30, y: 70 }, { x: 31, y: 70 }, { x: 70, y: 70 }]
...
describe('G11a-M2: the map grows, everything on it moves with it, and the log replays it', () => {
  ...
  it('carries every stored coordinate when the edge it widens is the north one', () => {
    ...
    const shifted = Object.keys(before.traffic!).map((k) => {
  ...
  it('the same run replays identically from genesis and from a pre-growth snapshot', () => {
    ...
    write([{ type: 'agent_spawned', payload: { id: 'walker', name: 'walker', x: 30, y: 70, ageDays: 7300 } }], 0)
...
describe('G11a-M3: an operator may move a law, and only the laws the whitelist names', () => {
  it('two flags flipped mid-run change the next tick, and the log replays the flips', () => {
    ...
    const replayed = changes.reduce<WorldState>((acc, p) => fold(acc, ev('config_changed', p, 404), CFG), s)
  ...
  it('a path the whitelist does not name is refused, by the fold and not by the caller', () => {
    ...
    expect(() => fold(s, ev('config_changed', { path: 'needs.hungerDecayPerTick', value: 0 }), CFG))
      .toThrow(/is not a world law/)
    // A whitelisted path with a value of the wrong shape is refused too.
    expect(() => fold(s, ev('config_changed', { path: 'thirst.enabled', value: 'yes' }), CFG))
...
describe('G11a-M4: the same taking, four times, and only the light tells them apart', () => {
  ...
  function scene(config: SimConfig, tick: number, torchAtTarget: boolean): WorldState {
    ...
    s = fold(s, ev('agent_spawned', { id: 'taker', name: 'taker', x: AT.x, y: AT.y, ageDays: 7300 }), config)
    s = fold(s, ev('agent_spawned', { id: 'owner', name: 'owner', x: AT.x, y: AT.y, ageDays: 7300 }), config)
    s = fold(s, ev('agent_spawned', {
      id: 'watcher', name: 'watcher', x: AT.x, y: AT.y + WATCH_DISTANCE, ageDays: 7300,
    }), config)
    s = fold(s, ev('item_spawned', {
      id: 'item_knife', kind: 'knife', qty: 1, loc: { t: 'tile', x: AT.x, y: AT.y }, owner: 'owner',
    }), config)
    if (torchAtTarget) {
      s = fold(s, ev('item_spawned', {
        id: 'item_torch', kind: 'torch', qty: 1, loc: { t: 'tile', x: AT.x, y: AT.y },
      }), config)
      s = fold(s, ev('item_lit', { itemId: 'item_torch', burnsUntilTick: tick + 100 }, tick - 1), config)
    }
    return { ...s, tick }
  }

  const taking: SimEvent = {
    seq: 999001, tick: 0, type: 'item_taken',
    payload: { itemId: 'item_knife', takerId: 'taker', ownerId: 'owner', kind: 'knife', x: AT.x, y: AT.y },
  }

  const witnessed = (s: WorldState, config: SimConfig): boolean =>
    composePerception(s, config, 'watcher', [{ ...taking, tick: s.tick }]).seen
      .some((x) => x.kind === 'item_taken')
...
describe('G11a-P1: the perf gate on a full 128x128 town', () => {
  ...
  function busyTown(): WorldState {
    ...
    for (let i = 0; i < AGENTS; i++) {
      s = fold(s, ev('agent_spawned', {
        id: `body_${i}`, name: `body_${i}`, x: 30 + (i % 6), y: 68 + Math.floor(i / 6), ageDays: 7300,
      }), CFG)
    }
    ...
    for (const [kind, cap] of Object.entries(CFG.fauna.caps)) {
      for (let i = living(kind); i < cap; i++) {
        ...
        s = fold(s, ev('fauna_spawned', {
          id: `fauna_${next++}`, kind, ...at, ...(kind === 'fish' ? { stock: 3 } : {}),
        }), CFG)
      }
    }
    return { ...s, tick: 599 }
  ...
  it('a mid-run pave is visible to the very next search — there is no cache to go stale', () => {
    ...
    for (let x = 1; x < 39; x++) {
      s = fold(s, ev('tile_changed', { x, y: 20, from: 0, to: 7, reason: 'paved', byId: 'mason' }), CFG)
    }
...
describe('G11a-D1: a competent body comes through three days on the default world, untouched', () => {
  ...
  function competent(): { state: WorldState; deaths: SimEvent[]; collapses: number } {
    ...
    for (let i = 0; i < 6; i++) {
      s = fold(s, ev('item_spawned', {
        id: `item_loaf_${i}`, kind: 'bread', qty: 1, loc: { t: 'agent', id: 'ada' },
      }), CFG)
    }
    for (let i = 0; i < 3; i++) {
      s = fold(s, ev('item_spawned', {
        id: `item_skin_${i}`, kind: 'waterskin', qty: 1, loc: { t: 'agent', id: 'ada' }, charges: 4,
      }), CFG)
    }
    ...
    for (let tick = 1; tick <= DAYS * MINUTES_PER_DAY; tick++) {
      ...
      const wants =
        minute === SLEEP_AT && !a.asleep ? { verb: 'sleep', params: {} }
          : minute === WAKE_AT && a.asleep ? { verb: 'wake', params: {} }
            : minute === EAT_AT ? { verb: 'eat', params: { itemId: loafInHand(s) } }
              : minute === DRINK_AT ? { verb: 'drink', params: { itemId: skinInHand(s) } }
                : null
      if (wants !== null && wants.params !== null) {
        const out = submitIntent({ ...s, tick }, CFG, 'ada', wants.verb, wants.params as Record<string, unknown>)
  ...
  const loafInHand = (s: WorldState): string =>
    Object.keys(s.items).sort().find((id) => s.items[id]!.kind === 'bread'
      && s.items[id]!.loc.t === 'agent') ?? 'item_loaf_0'

  const skinInHand = (s: WorldState): string =>
    Object.keys(s.items).sort().find((id) => s.items[id]!.kind === 'waterskin'
      && (s.items[id]!.charges ?? 0) > 0) ?? 'item_skin_0'
  ...
  it('one collapse leaves a fatigue clock that a full night of sleep does not lift', () => {
    ...
    expect(rested.agents.ada!.afflictions).toEqual([
      { kind: 'fatigue', severity: 1, sinceTick: 1 },
    ])

    // A herb is the one road back the world actually has.
    let cured = fold(rested, ev('item_spawned', {
      id: 'item_herb', kind: 'herb', qty: 1, loc: { t: 'agent', id: 'ada' },
    }, rested.tick), CFG)
    const awake = fold(cured, ev('agent_woke', { agentId: 'ada' }, cured.tick), CFG)
    const eaten = submitIntent({ ...awake, tick: awake.tick + 1 }, CFG, 'ada', 'eat', { itemId: 'item_herb' })
    expect(eaten.ok).toBe(true)
    cured = apply({ ...awake, tick: awake.tick + 1 }, CFG, (eaten as { events: PendingEvent[] }).events, awake.tick + 1)
