...
7. "What just happened" cue: the stage cue slot prints `discovery_made`, `law_*`, `custom` and bond-change events for 6 s with a 16 px pixel icon, then fades; the two involved bodies bounce (reuse the finished-structure bounce). Test: the cue text appears within one frame of the event and clears after 6 s.
   7b. (Owner, 2026-09-02, from mock A "Alive": "I absolutely loved… icons appear above characters' heads when they achieve something, or a status shows below them when they are doing something.") Emotes rise off the head of every involved body: a pixel heart for a bond gained, a crack for a bond lost, a lit bulb for a discovery, law or custom, an ember for a ruling amended; 1.8 s rise with ease-out, fade over the last 0.6 s, drawn in world space above the overhead slot so the two never collide. The act chip under the feet keeps its gerund and gains a 1 px honey progress bar beneath it; the seven progress blocks over the head are removed. Test: an emote spawns per involved body with the right glyph and is gone after 1.8 s; the progress bar width tracks the act's fraction; no progress blocks remain in the overhead layer.
