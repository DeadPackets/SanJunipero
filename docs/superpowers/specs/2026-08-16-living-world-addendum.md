...
3. ≥ 1 novel intent adjudicated and codified live; a repeat of the same intent resolves with **0 arbiter LLM calls** (call-count assertion). **CLOSED BY USER RULING (2026-08-17), not by a live pass.** Five G9b runs never codified. Run 5 diagnosed the last cause exactly: the frontier reached the arbiter and five `attempt` verdicts came back, and the deterministic `withinAdjacency` gate destroyed all five because nothing in the prompt bound `recipe.canon` to the ids the context had just listed, so the model invented plausible ones. The user ruled, verbatim: *"Add it, but no need for a run. The run will naturally happen in the next stages of testing."* The defect is fixed at commit `c8d267b` (the canon-vocabulary instruction line, unit-tested through prompt assembly and the Esen adjudicator fixture); live verification **transfers to the C8 dress rehearsal**, whose criteria must include a codification check — see the amendment in `docs/superpowers/plans/c8-delta-from-c9.md` §8. `checkG9Report`'s criterion 3 is deliberately left biting, so `pnpm test:live` against run 5's archived report stays red on this row alone: the criterion is not retired, its verification is deferred.
4. **The novel-intent route fires live, end to end** (RESTATED, user-approved 2026-08-17). Originally: "an `unknown verb:` rejection observed routing to the arbiter instead of surfacing as refusal prose". G9b run 4 showed that wording rewards incoherence: a fed, articulate town invents no verb *names* — it asks through the doors CAPABILITIES advertises (`craft {recipe}`, `build {kind}`), so it scored zero while proposing new physics all run, and run 1 scored the criterion only because the minds were flailing. Satisfied now by **either** a live codification (the §17.3 event *is* the route completing) **or** an unknown-verb / unknown-recipe routing that reaches the arbiter — and in both cases zero proposals may die as refusal prose in a mind's memory. Verb-spam no longer scores; coherent novelty does.
